...
def scan(
    ...
    inventory_ndjson_path = os.path.join(scan_folder, "inventory.ndjson") if stream_results and not use_account_region_model else None
    if stream_results and not use_account_region_model:
        # Ensure files exist
        with open(results_ndjson_path, "a", encoding="utf-8"):
            pass
        with open(inventory_ndjson_path, "a", encoding="utf-8"):
            pass

    def _write_result(result: Dict[str, Any]) -> None:
        if not stream_results or use_account_region_model:
            return
        # Always write inventory to inventory.ndjson (best-effort)
        _write_inventory_assets(result, inventory_ndjson_path)
    ...
    if use_flattened_model:
        ...
        for account in accounts_to_scan:
            ...
            for service_name, scope in global_services:
                all_service_tasks.append({
                    ...
                    'inventory_file': os.path.join(scan_folder, f"inventory_{account_id}_global.ndjson"),
                })
            ...
            for region in regions_to_scan:
                for service_name, scope in regional_services:
                    all_service_tasks.append({
                        ...
                        'inventory_file': os.path.join(scan_folder, f"inventory_{account_id}_{region}.ndjson"),
                    })
        ...
        def _write_service_result(task: Dict, result: Dict[str, Any]) -> None:
            ...
            with file_lock:
                os.makedirs(os.path.dirname(task['inventory_file']), exist_ok=True)
                # Touch file to ensure it exists
                with open(task['inventory_file'], "a", encoding="utf-8"):
                    pass
            ...
            for discovery_id, items in inventory.items():
                ...
                for item in items:
                    ...
                    try:
                        ...
                        with file_lock:
                            with open(task['inventory_file'], "a", encoding="utf-8") as fh:
                                fh.write(json.dumps(asset, default=str) + "\n")
        ...
        for task in all_service_tasks:
            ...
            if task['inventory_file'] not in inventory_files_by_key.get(key, []):
                if key not in inventory_files_by_key:
                    inventory_files_by_key[key] = []
                inventory_files_by_key[key].append(task['inventory_file'])
        ...
        inventory_files_list = [f for files in inventory_files_by_key.values() for f in files]
        ...
        for key, files in results_files_by_key.items():
            account_region_summaries.append({
                ...
                "inventory_file": inventory_files_by_key.get(key, [None])[0],
            })
    ...
    # NEW ACCOUNT+REGION MODEL
        ...
        for account in accounts_to_scan:
            for region in regions_to_scan:
                account_region_tasks.append({
                    ...
                    'inventory_file': os.path.join(scan_folder, f"inventory_{account['Id']}_{region}.ndjson"),
                })
        ...
        for account in accounts_to_scan:
            global_tasks.append({
                ...
                'inventory_file': os.path.join(scan_folder, f"inventory_{account['Id']}_global.ndjson"),
            })
        ...
        summary["inventory_files"] = [s.get("inventory_file") for s in account_region_summaries if s.get("inventory_file")]
    ...
    if use_flattened_model or use_account_region_model:
        ...
        summary["inventory_files"] = inventory_files_list
        summary["account_region_summaries"] = account_region_summaries
        ...
        if os.getenv("WRITE_CONSOLIDATED_NDJSON", "1").strip() not in ("0", "false", "False"):
            ...
            consolidated_inventory = os.path.join(scan_folder, "inventory.ndjson")
            ...
            try:
                ...
                _concat_ndjson(consolidated_inventory, inventory_files_list)
                summary["results_file"] = consolidated_results
                summary["inventory_file"] = consolidated_inventory
                logger.info(f"Consolidated results written to: {consolidated_results}")
                logger.info(f"Consolidated inventory written to: {consolidated_inventory}")
            except Exception as e:
...
def scan_account_region_scope(
    ...
    Args:
        ...
        inventory_file: Path to write inventory.ndjson
    ...
    with open(results_file, "a", encoding="utf-8"):
        pass
    with open(inventory_file, "a", encoding="utf-8"):
        pass
        ...
        for discovery_id, items in inventory.items():
            ...
            for item in items:
                ...
                try:
                    ...
                    # Append to inventory file
                    with file_lock:
                        with open(inventory_file, "a", encoding="utf-8") as fh:
                            fh.write(json.dumps(asset, default=str) + "\n")
