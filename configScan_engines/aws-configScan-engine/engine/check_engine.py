...
class CheckEngine:
    ...
    def _load_discoveries_from_ndjson(self, scan_id: str, discovery_id: str, 
        ...
        # Try new structure first: configscan/discoveries/
        discoveries_dir = base_output_dir / "configscan" / "discoveries" / scan_id / "discovery"
    ...
    def _load_discoveries_from_database(self, discovery_id: str, tenant_id: str,
                                       hierarchy_id: str, scan_id: str) -> List[Dict]:
        """
        Load discoveries from database (production mode)
        ...
        Returns:
            List of discovery items from database
        """
        if not self.db:
            raise ValueError("DatabaseManager not initialized")
        
        return self.db.query_discovery(
    ...
    def run_check_scan(self, scan_id: str, customer_id: str, tenant_id: str,
        ...
        base_output_dir = Path("/Users/apple/Desktop/threat-engine/engines-output/aws-configScan-engine/output") / "configscan"
        discoveries_dir = base_output_dir / "discoveries" / scan_id
        rule_check_dir = base_output_dir / "rule_check" / f"rule_check_{check_scan_id}_{output_timestamp}"
        if discoveries_dir.exists():
            output_dir = rule_check_dir
        else:
            output_dir = rule_check_dir
        ...
        for service_idx, service in enumerate(services, 1):
            ...
            try:
                # Load checks YAML
                checks_file = Path(f"services/{service}/checks/{check_source}/{service}.checks.yaml")
    ...
    def _export_check_results_to_file(self, check_scan_id: str, discovery_scan_id: str,
        ...
        if output_dir is None:
            ...
            output_dir = base_output_dir / "rule_check" / f"rule_check_{check_scan_id}_{timestamp}"
        ...
        # Export as NDJSON
        findings_file = output_dir / "findings.ndjson"
        ...
        logger.info(f"  Exported {len(results)} check results to {findings_file.name}")
