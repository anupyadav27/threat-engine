...
def is_cspm_inventory_resource(discovery_id: str, discovery_config: Dict[str, Any] = None) -> bool:
    """
    Universal heuristic to determine if a discovery operation should produce inventory assets.
    ...
    Returns:
        True if should produce inventory assets
    """
    import re
...
def save_reporting_bundle(results: List[Dict[str, Any]], account_id: str | None = None, scan_folder: str = None) -> str:
    ...
    # Extract inventory assets using generic approach
    try:
        inventory_path = extract_inventory_assets(results, folder, account_id)
        logger.info(f"Inventory assets extracted to: {inventory_path}")
    except Exception as e:
        logger.warning(f"Failed to extract inventory assets: {e}")
    
    # Always emit inventory.ndjson (best-effort). This is consumed by Inventory/Threat engines.
    try:
        inventory_path = extract_inventory_assets(results, folder, account_id)
        logger.info(f"Generated inventory assets: {inventory_path}")
    except Exception as e:
        logger.warning(f"Failed to generate inventory assets: {e}")
...
def extract_inventory_assets(
    ...
    """
    Extract inventory assets from scan results using generic approach.
    ...
    Args:
        ...
        scan_folder: Path to scan folder where inventory.ndjson will be saved
        account_id: Default account ID if not found in results
    
    Returns:
        Path to inventory.ndjson file
    """
            ...
            for item in items:
                ...
                try:
                    ...
                    if '_dependent_data' in item:
                        logger.debug(f"[INVENTORY-WRITE] Found _dependent_data in item for {name}: type={type(item['_dependent_data']).__name__}")
                        if isinstance(item['_dependent_data'], dict):
                            ...
                            if enriched_discoveries:
                                ...
                                if isinstance(first_data, dict):
                                    logger.debug(f"[INVENTORY-WRITE] Sample data from {first_disc}: {len(first_data)} fields - {list(first_data.keys())[:5]}")
                        else:
                            logger.warning(f"[INVENTORY-WRITE] _dependent_data is not a dict for {name}: {type(item['_dependent_data']).__name__}")
                    else:
                        logger.debug(f"[INVENTORY-WRITE] No _dependent_data in item for {name} (item keys: {list(item.keys())[:10]})")
