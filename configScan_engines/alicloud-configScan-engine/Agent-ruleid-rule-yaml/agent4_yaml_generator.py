...
def generate_discovery_section(service: str, resource: str, operation: str, validated_op: Dict) -> Dict:
    """Generate discovery section for Alibaba"""
    discovery_id = f"alicloud.{service}.{operation}"
    ...
    discovery = {
        ...
        'emit': {
            'items_for': f'{{{{ {operation}_response.items }}}}',  # Alibaba typically uses 'items'
            'as': 'item',
            'item': {}
        }
    }
...
def generate_check_section(requirement: Dict) -> Dict:
    ...
    for field_req in ai_reqs.get('fields', []):
        alicloud_field = field_req.get('alicloud_api_field', '')
        operator = field_req.get('operator', 'equals')
        expected_value = field_req.get('alicloud_api_field_expected_values')
        
        # Validate field exists
        field_validation = requirement.get('field_validation', {}).get(alicloud_field, {})
        if not field_validation.get('exists'):
            continue
        
        condition = {
            'var': f'item.{alicloud_field}',
            'op': operator
        }
...
def generate_yaml_for_service(service: str, requirements: List[Dict]) -> Dict:
    ...
    for req in requirements:
        ...
        if discovery_id not in added_discoveries:
            discovery = generate_discovery_section(service, resource, operation, validated_op)
...
def main():
    logger.info("Agent 4: Alibaba YAML Generator starting")
    print("=" * 80)
    print("AGENT 4: Alibaba YAML Generator")
    print("=" * 80)
    print()
    ...
    for service, requirements in all_requirements.items():
        print(f"\n📦 {service}")
        logger.info(f"Generating YAML for service: {service}")
    ...
    print(f"   Output: output/{{service}}_generated.yaml")
    print("=" * 80)
