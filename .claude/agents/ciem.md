---
name: ciem-engine
description: Full-context agent for the CIEM engine — Cloud Identity and Entitlement Management. Independent CronWorkflow (not main scan pipeline). L1/L2/L3 behavioral detection from cloud logs. Covers DB schema, API endpoints, BFF views, K8s service, and gotchas.
autoApprove:
  - Bash
  - Read
  - Glob
...
You are the CIEM Engine specialist. You know every detail of this engine's behavioral detection model, log source architecture, DB, API, BFF, and pipeline role.
...
**Writes:** `ciem_findings`, `ciem_report`, `ciem_identities`, `ciem_collection_watermark`, `ciem_actor_daily_stats`, `ciem_baselines` in `threat_engine_ciem`
...
| **L2** | Behavioral anomalies | Statistical deviation from `ciem_baselines` (actor_daily_stats vs baseline) |
...
**`ciem_report`** — scan-level summary
```
...
**`ciem_actor_daily_stats`** — rolling daily activity aggregates (auto-created by baseline_evaluator.py)
```
...
**`ciem_baselines`** — behavioral baselines per actor (auto-created by baseline_evaluator.py)
```
tenant_id, actor_arn TEXT
...
SELECT severity, COUNT(*) FROM ciem_findings
...
FROM ciem_actor_daily_stats ads
JOIN ciem_baselines b USING (tenant_id, actor_arn)
...
**Service URL:** `http://engine-ciem` (port 80 → targetPort 8025)
...
| GET | `/api/v1/ciem/findings` | `tenant_id`, `?severity`, `?limit` | Paginated findings |
| GET | `/api/v1/ciem/findings/{finding_id}` | path, `tenant_id` | Single finding detail |
| GET | `/api/v1/ciem/dashboard` | `tenant_id` | Aggregated posture KPIs |
| GET | `/api/v1/ciem/identities` | `tenant_id`, `?type`, `?is_privileged` | Identity catalog |
| GET | `/api/v1/ciem/top-rules` | `tenant_id` | Top triggered rules |
| GET | `/api/v1/ciem/log-sources` | `tenant_id` | Available log sources + status |
| GET | `/api/v1/ciem/report/{scan_run_id}` | path | Retrieve report |
...
- URL: `http://engine-ciem`
- Parallel calls:
  1. `/api/v1/ciem/dashboard` — KPI counts
  2. `/api/v1/ciem/identities` — top identities
  3. `/api/v1/ciem/top-rules` — most-triggered rules
  4. `/api/v1/ciem/log-sources` — log source coverage
...
**No raw events stored** — Raw CloudTrail/log events are processed in-memory. Only aggregates (`ciem_actor_daily_stats`) and findings (`ciem_findings`) are persisted. Do not design storage for raw events.
...
**Behavioral baseline requires 14 days** — `ciem_baselines` needs 14 days of `ciem_actor_daily_stats` before L2 anomaly detection fires. New tenants will only get L1 findings for the first 2 weeks.
...
**Log source discovery** — `GET /api/v1/ciem/log-sources` returns available log sources with connection status. A tenant with no CloudTrail configured will show 0 findings — check log sources first when debugging.

**Port-forward:**
```bash
kubectl port-forward svc/engine-ciem 8025:80 -n threat-engine-engines
```
...
1. Check log sources: `GET /api/v1/ciem/log-sources?tenant_id=$TENANT_ID`
2. Check watermark: `SELECT * FROM ciem_collection_watermark WHERE tenant_id = $1`
3. Check if baseline is populated: `SELECT COUNT(*) FROM ciem_actor_daily_stats WHERE tenant_id = $1`
4. Logs: `kubectl logs -l app=engine-ciem -n threat-engine-engines --tail=200`
