# STORY-GATEWAY-01: Asset Context Aggregator Endpoint

## Track
Investigation Flow — Critical Path Sprint

## Priority
P0 — STORY-UI-01 (panel cross-engine section) and STORY-UI-02 (AssetContextCard) both block on this

## Story
As a security analyst clicking a finding row, I need the side panel to show lightweight summaries from every other engine for the same resource, so I can see the full blast radius in one place without navigating away.

## Current State

`shared/api_gateway/main.py` proxies all engine traffic via `PROXY_MAP` and mounts the BFF views router at `/api/v1/views/*`. There is no endpoint that aggregates cross-engine findings for a single `resource_uid`. The existing `FindingDetailPanel` receives only the row data passed to it — no cross-engine data is fetched.

## Files to Modify / Create

- `shared/api_gateway/asset_context.py` — **NEW** — FastAPI router for the aggregator endpoint
- `shared/api_gateway/main.py` — mount the new router (2 lines, after BFF views mount ~line 332)

## Exact Changes

### 1. `shared/api_gateway/asset_context.py` — new file
...
### 2. `shared/api_gateway/main.py` — add after BFF views router mount (~line 332)
...
### 3. Engine-side prerequisite (tracked in STORY-GATEWAY-02)
...
```json
{
  "findings": [
    {
      "finding_id": "...", "title": "...", "severity": "critical",
      "status": "FAIL", "rule_id": "...", "resource_uid": "...",
...
```
The gateway degrades to `available=False` for any engine that has not yet implemented this endpoint (405 or timeout).

## Acceptance Criteria

- [ ] `GET /gateway/api/v1/asset-context/{resource_uid}` returns HTTP 200
- [ ] Response contains all 11 engine keys (`check`, `network`, `iam`, `datasec`, `encryption`, `threat`, `vulnerability`, `container`, `dbsec`, `ai_security`, `ciem`)
- [ ] Engine that times out → `{ "available": false, "finding_count": 0, "top_findings": [] }` — never causes 500
- [ ] Engine returning 404 for `resource_uid` → `{ "available": true, "finding_count": 0, "top_findings": [] }`
- [ ] Engine returning 405 (not implemented yet) → `available: false`
- [ ] `top_findings` contains max 3 items, sorted by severity (critical first)
- [ ] `max_severity` is `null` when `finding_count == 0`
- [ ] `tenant_id` derived from `X-Auth-Context` header — not from query param in production
- [ ] Total wall-clock latency ≤ 2.2 s under normal engine load
- [ ] Route mounted at both `/api/v1/asset-context/{uid}` and `/gateway/api/v1/asset-context/{uid}`

## Security Checklist

- [ ] `tenant_id` scoped from `X-Auth-Context` — query param only used as local dev fallback
- [ ] `resource_uid` path param passed to engine via httpx params (URL-encoded) — never string-interpolated into SQL
- [ ] `credential_ref` never returned in response (engines should exclude it; verify)
- [ ] No engine-internal service URLs exposed in response body

## Definition of Done

- [ ] Endpoint returns correct shape with real data from at least 3 live engines in dev cluster
- [ ] Query a known `resource_uid` with check + network findings → both appear with correct counts
- [ ] Query an unknown `resource_uid` → all engines return `available=true, finding_count=0`
- [ ] Kill one engine pod → that engine returns `available=false`, others unaffected
- [ ] No hardcoded `tenant_id` in implementation
