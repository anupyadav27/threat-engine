...
def is_read_operation_by_name(op_name: str) -> bool:
    """Check if operation name suggests it's a read operation.
    Based on IBM patterns:
    ...
    read_prefixes = ['list_', 'get_', 'describe_', 'read_', 'fetch_', 
                     'search_', 'query_', 'find_', 'enumerate_']
    
    return any(op_lower.startswith(prefix) for prefix in read_prefixes)

def build_fallback_dependency_index_from_ibm(service_name: str, service_dir: Path) -> Optional[Dict]:
    """Build a minimal dependency_index from ibm_dependencies when dependency_index is empty.
    Includes operations that:
    1. Have item_fields (preferred), OR
    2. Have names that suggest read operations (list_*, get_*, describe_*, etc.)
    """
    
    ibm_file = service_dir / "ibm_dependencies_with_python_names_fully_enriched.json"
    ...
    service_data = ibm_data.get(service_name, {})
    
    # IBM has both 'independent' and 'dependent' arrays, plus 'operations' array
    all_operations = []
    all_operations.extend(service_data.get("independent", []))
    all_operations.extend(service_data.get("dependent", []))
    all_operations.extend(service_data.get("operations", []))
    ...
    for op_data in all_operations:
        ...
        # Include operations that:
        # 1. Have item_fields (read operations with field definitions), OR
        # 2. Have names that suggest read operations (list_*, get_*, etc.)
        item_fields = op_data.get("item_fields", {})
        is_read_by_name = is_read_operation_by_name(op_name)
        
        if not item_fields and not is_read_by_name:
            continue  # Skip non-read operations
        
        # Determine if this is an independent (root) operation
        # list_* operations are typically independent
        # get_* operations might be dependent (need an ID)
        is_independent = op_name.lower().startswith(('list_', 'search_', 'query_', 'find_', 'enumerate_'))
        
        # Extract resource name from operation for entity naming
        # e.g., list_accounts -> accounts, get_account -> account
        resource_name = op_name.lower()
        if resource_name.startswith('list_'):
            resource_name = resource_name[5:]  # Remove 'list_'
        elif resource_name.startswith('get_'):
            resource_name = resource_name[4:]  # Remove 'get_'
        elif resource_name.startswith('describe_'):
            resource_name = resource_name[9:]  # Remove 'describe_'
        elif resource_name.startswith('read_'):
            resource_name = resource_name[5:]  # Remove 'read_'
        elif resource_name.startswith('fetch_'):
            resource_name = resource_name[6:]  # Remove 'fetch_'
        elif resource_name.startswith('search_'):
            resource_name = resource_name[7:]  # Remove 'search_'
        elif resource_name.startswith('query_'):
            resource_name = resource_name[6:]  # Remove 'query_'
        elif resource_name.startswith('find_'):
            resource_name = resource_name[5:]  # Remove 'find_'
        elif resource_name.startswith('enumerate_'):
            resource_name = resource_name[10:]  # Remove 'enumerate_'
        
        # Create entity name: ibm.service.resource_name
        entity_name = f"ibm.{service_name}.{resource_name}"
        
        if is_independent:
            # Create a root operation
            roots.append({
                "op": op_name,
                "produces": [entity_name]
            })
        
        # Create entity path
        if entity_name not in entity_paths:
            entity_paths[entity_name] = []
        
        entity_paths[entity_name].append({
            "operations": [op_name],
            "produces": {op_name: [entity_name]},
            "consumes": {},
            "external_inputs": []
        })
...
def generate_operations_report(service_name: str, service_dir: Path) -> Optional[Dict]:
    ...
    # If dependency_index is empty, try to build from ibm_dependencies
    if dependency_index is None:
        dependency_index = build_fallback_dependency_index_from_ibm(service_name, service_dir)
        if dependency_index is None:
...
def generate_all_services(base_dir: Path):
    ...
    print("="*80)
    print("GENERATING MINIMAL OPERATIONS LIST FOR ALL IBM SERVICES")
    print("="*80)
    
    # Get all service directories - include those with dependency_index OR ibm_dependencies
    service_dirs = [d for d in base_dir.iterdir() 
                    if d.is_dir() and not d.name.startswith('.') 
                    and ((d / 'dependency_index.json').exists() or 
                         (d / 'ibm_dependencies_with_python_names_fully_enriched.json').exists())]
...
def main():
    base_dir = Path('/Users/apple/Desktop/threat-engine/pythonsdk-database/ibm')
    
    import sys
    if len(sys.argv) > 1 and sys.argv[1] == '--all':
        generate_all_services(base_dir)
    else:
        # Generate for single service (iam) as test
        service_name = 'iam'
