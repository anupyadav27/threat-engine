#!/usr/bin/env python3
"""
Validate KEYSPACES YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "EncryptionSpecification.EncryptionType" with items_for -> "item.EncryptionSpecification.EncryptionType"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('Keyspaces.'):
            clean_path = clean_path.replace('Keyspaces.', '')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('keyspaces_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'keyspaces.yaml'
