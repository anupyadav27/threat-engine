...
class RuleMapper:
    ...
    def get_controls_for_rule(self, rule_id: str, csp: str, use_metadata: bool = False) -> List[FrameworkControl]:
        """
        ...
        cache_key = f"{csp}_{use_metadata}"
        if cache_key not in self._mappings_cache:
            self._mappings_cache[cache_key] = self.framework_loader.get_rule_mappings(csp, use_metadata=use_metadata)
        
        csp_mappings = self._mappings_cache[cache_key]
        controls = csp_mappings.get(rule_id, [])
        
        # If no controls found and use_metadata, try loading directly from metadata
        if not controls and use_metadata:
            try:
                from ..loader.metadata_loader import MetadataLoader
                metadata_loader = MetadataLoader()
                controls = metadata_loader.get_compliance_mappings(rule_id)
                if controls:
                    # Cache it
                    csp_mappings[rule_id] = controls
            except Exception:
                pass
