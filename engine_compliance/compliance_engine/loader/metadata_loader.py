    ...
    def __init__(self, rule_db_path: Optional[Path] = None):
        ...
        Args:
            rule_db_path: Base path to rule_db
                         Default: engine_input/engine_configscan_aws/input/rule_db/default/services
        """
        if rule_db_path is None:
            from engine_common.storage_paths import get_project_root
            root = get_project_root()
            rule_db_path = root / "engine_input" / "engine_configscan_aws" / "input" / "rule_db" / "default" / "services"
