    ...
    def __init__(self, discovery_base_path: Optional[str] = None):
        ...
        Args:
            discovery_base_path: Base path to discovery files.
                               Default: engines-output/aws-configScan-engine/output/discoveries
        """
        if discovery_base_path is None:
            # Default to engines-output relative to project root (threat-engine)
            # Path: .../threat-engine/inventory-engine/inventory_engine/connectors/discovery_reader.py
            # Go up 4 levels to get to threat-engine
            base_path = Path(__file__).parent.parent.parent.parent
            self.discovery_base_path = base_path / "engines-output" / "aws-configScan-engine" / "output" / "discoveries"
        else:
