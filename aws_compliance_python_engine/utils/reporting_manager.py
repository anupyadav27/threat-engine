...
def save_reporting_bundle(results: List[Dict[str, Any]], account_id: str | None = None) -> str:
    ...
    with open(os.path.join(folder, "inventories.json"), "w") as fh:
        json.dump({"metadata": _meta(account_id, folder), "inventories": inventories}, fh, indent=2)
    with open(os.path.join(folder, "main_checks.json"), "w") as fh:
        json.dump({"metadata": _meta(account_id, folder), "checks": checks_main}, fh, indent=2)
    with open(os.path.join(folder, "skipped_checks.json"), "w") as fh:
        json.dump({"metadata": _meta(account_id, folder), "checks": checks_skipped}, fh, indent=2)
