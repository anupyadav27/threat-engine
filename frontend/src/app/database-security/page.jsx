...
import { buildUniversalColumns, PublicAccessCell } from '@/components/shared/EngineTableCells';
...
export default function DatabaseSecurityPage() {
  ...
  const findingsColumns = useMemo(() => buildUniversalColumns('database-security', [
    {
      accessorKey: 'db_service',
      header: 'DB Service',
      size: 90,
      cell: (info) => {
        const v = info.getValue();
        return v
          ? <span className="text-xs px-2 py-0.5 rounded font-medium"
              style={{ backgroundColor: 'rgba(20,184,166,0.12)', color: '#2dd4bf' }}>{v}</span>
          : null;
      },
    },
    {
      ...
      header: 'Access',
      size: 68,
      accessorFn: row => row.publicly_accessible ?? row.public_access ?? null,
      cell: ({ row }) => <PublicAccessCell row={row.original} />,
    },
  ]), []);
  ...
  return (
    ...
    >
      ...
      <FindingDetailPanel finding={selectedFinding} onClose={() => setSelectedFinding(null)} context={{ engine: 'database-security', allFindings: findings }} />
    </EngineShell>
  );
}
