...
export default function CompliancePage() {
  ...
  const REMEDIATION = {
    critical: ['Immediately revoke or restrict access', 'Apply least-privilege IAM policy', 'Enable MFA for all console access', 'Rotate all exposed credentials within 24h', 'Create incident response ticket'],
    high:     ['Schedule remediation within 7 days', 'Review and tighten resource policies', 'Enable logging and monitoring for this service', 'Apply automated compliance rule fix'],
    medium:   ['Remediate within 30 days', 'Review configuration against CIS benchmark', 'Enable recommended security controls', 'Update runbook with fix procedure'],
    low:      ['Schedule for next sprint', 'Review against security baseline', 'Document accepted risk if not fixing'],
  };

  const DESCRIPTIONS = {
    'S3':  'Amazon S3 bucket is publicly accessible or misconfigured, potentially exposing sensitive data to unauthorized access.',
    'IAM': 'IAM configuration does not meet security best practices, allowing excessive permissions or weak authentication.',
    'RDS': 'RDS database instance has a security misconfiguration that could expose data or allow unauthorized connections.',
    'CloudTrail': 'CloudTrail logging is not properly configured, creating gaps in audit trail and compliance evidence.',
    'MFA': 'Multi-factor authentication is not enforced, increasing the risk of unauthorized access via compromised credentials.',
    'TLS': 'Transport Layer Security is not enforced, allowing data to be transmitted in plaintext over the network.',
    'default': 'This control is failing and requires remediation to meet the framework requirements and reduce security risk.',
  };

  const getDescription = (control) => {
    if (!control) return '';
    const t = (control.title || '').toUpperCase();
    for (const [key, desc] of Object.entries(DESCRIPTIONS)) {
      if (t.includes(key.toUpperCase())) return desc;
    }
    return DESCRIPTIONS.default;
  };

  const ControlDetailDrawer = ({ control, onClose }) => {
    if (!control) return null;
    const sc    = SEV_CFG[control.severity] || SEV_CFG.medium;
    const desc  = getDescription(control);

    // Fetch real affected resources from compliance DB
    ...
    return (
      <>
        {/* Backdrop */}
        <div onClick={onClose} style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.45)', zIndex: 40, backdropFilter: 'blur(2px)' }} />
        {/* Drawer */}
        <div style={{
          position: 'fixed', top: 0, right: 0, bottom: 0, width: 520, zIndex: 50,
          backgroundColor: 'var(--bg-card)', borderLeft: '1px solid var(--border-primary)',
          overflowY: 'auto', display: 'flex', flexDirection: 'column',
          boxShadow: '-8px 0 32px rgba(0,0,0,0.3)',
        }}>
          {/* Header */}
          <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border-primary)', backgroundColor: 'var(--bg-secondary)' }}>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 10 }}>
              <div>
                <code style={{ fontSize: 12, padding: '3px 8px', borderRadius: 6, backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-tertiary)', fontFamily: 'monospace' }}>{control.control_id}</code>
                <span style={{ marginLeft: 8, fontSize: 11, padding: '2px 8px', borderRadius: 20, fontWeight: 700, backgroundColor: sc.bg, color: sc.text }}>{control.severity?.toUpperCase()}</span>
              </div>
              <button onClick={onClose} style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid var(--border-primary)', backgroundColor: 'transparent', color: 'var(--text-secondary)', cursor: 'pointer', fontSize: 16, lineHeight: 1 }}>✕</button>
            </div>
            <h2 style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', margin: 0, lineHeight: 1.4 }}>{control.title}</h2>
            <div style={{ display: 'flex', gap: 12, marginTop: 8, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Framework: {control.framework}</span>
              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Account: {control.account || '—'}</span>
              <span style={{ fontSize: 11, color: control.days_open > 30 ? C.failed : C.amber }}>{control.days_open}d open</span>
            </div>
          </div>
          ...
          <div style={{ padding: 24, flex: 1, display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            {/* Description */}
            <section>
              <h3 style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>Description</h3>
              <p style={{ fontSize: 13, color: 'var(--text-primary)', lineHeight: 1.6, margin: 0, padding: '12px 14px', borderRadius: 8, backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-primary)' }}>
                {desc}
              </p>
            </section>

            {/* Finding Info (from first real resource) */}
            {resources[0] && (
              <section>
                <h3 style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>Finding Details</h3>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, padding: '12px 14px', borderRadius: 8, backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-primary)' }}>
                  <div><span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Rule ID</span><br/><code style={{ fontSize: 10, color: 'var(--text-secondary)' }}>{resources[0].rule_id}</code></div>
                  <div><span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Category</span><br/><span style={{ fontSize: 12, color: 'var(--text-primary)' }}>{resources[0].category}</span></div>
                  <div><span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Confidence</span><br/><span style={{ fontSize: 12, color: 'var(--text-primary)' }}>{resources[0].confidence}</span></div>
                  <div><span style={{ fontSize: 10, color: 'var(--text-muted)' }}>First Seen</span><br/><span style={{ fontSize: 12, color: 'var(--text-primary)' }}>{resources[0].first_seen ? new Date(resources[0].first_seen).toLocaleDateString() : '—'}</span></div>
                </div>
              </section>
            )}

            {/* Affected Resources — real data from DB */}
            <section>
              <h3 style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>
                Affected Resources <span style={{ fontSize: 10, fontWeight: 400, textTransform: 'none', letterSpacing: 0, padding: '1px 6px', borderRadius: 10, backgroundColor: `${sc.text}20`, color: sc.text }}>{loadingRes ? '...' : resources.length}</span>
              </h3>
              {loadingRes ? (
                <div style={{ padding: 16, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>Loading resources...</div>
              ) : resources.length === 0 ? (
                <div style={{ padding: 16, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12 }}>No affected resources found</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 300, overflowY: 'auto' }}>
                  {resources.map((r, i) => (
                    <div key={r.finding_id || i} style={{ padding: '10px 12px', borderRadius: 8, backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-primary)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 2 }}>{r.resource_type} · {r.region || 'global'}</div>
                          <div style={{ fontSize: 11, color: 'var(--text-primary)', fontFamily: 'monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.resource_uid || r.resource_arn}</div>
                        </div>
                        <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, backgroundColor: sc.bg, color: sc.text, flexShrink: 0 }}>{r.check_result || 'FAIL'}</span>
                      </div>
                      {/* Evidence */}
                      {r.checked_fields?.length > 0 && (
                        <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                          {r.checked_fields.map((cf, ci) => (
                            <code key={ci} style={{ fontSize: 9, padding: '1px 5px', borderRadius: 4, backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-tertiary)' }}>{cf}</code>
                          ))}
                        </div>
                      )}
                      {r.actual_values && Object.keys(r.actual_values).length > 0 && (
                        <pre style={{ marginTop: 4, fontSize: 9, padding: '4px 8px', borderRadius: 4, backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-tertiary)', overflow: 'auto', maxHeight: 80 }}>
                          {JSON.stringify(r.actual_values, null, 2)}
                        </pre>
                      )}
                    </div>
                  ))}
            ...
            {/* Remediation */}
            {resources[0]?.remediation && (
              <section>
                <h3 style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>Remediation</h3>
                <p style={{ fontSize: 13, color: 'var(--text-primary)', lineHeight: 1.6, margin: 0, padding: '12px 14px', borderRadius: 8, backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-primary)' }}>
                  {resources[0].remediation}
                </p>
              </section>
            ...
            {/* Status */}
            <section>
              <h3 style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 8 }}>Status</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {[
                  { label: 'First Detected', value: `${control.days_open} days ago`, color: C.failed },
                  { label: 'Last Seen',       value: resources[0]?.last_seen ? new Date(resources[0].last_seen).toLocaleDateString() : 'Today', color: 'var(--text-muted)' },
                  { label: 'Status',          value: 'Open — Awaiting Remediation',  color: C.amber },
                  { label: 'Framework',       value: control.framework,              color: C.blue },
                ].map(({ label, value, color }) => (
                  <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--border-primary)' }}>
          ...
          {/* Footer Actions */}
          <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border-primary)', backgroundColor: 'var(--bg-secondary)', display: 'flex', gap: 8 }}>
            {[
              { label: 'Create Ticket',  bg: 'var(--accent-primary)', text: 'white' },
              { label: 'Add Exception',  bg: 'var(--bg-tertiary)',    text: 'var(--text-primary)' },
              { label: 'Acknowledge',     bg: 'var(--bg-tertiary)',    text: 'var(--text-primary)' },
            ].map(({ label, bg, text }) => (
              <button key={label} onClick={onClose} style={{ flex: 1, padding: '8px 4px', borderRadius: 8, fontSize: 12, fontWeight: 600, backgroundColor: bg, color: text, border: '1px solid var(--border-primary)', cursor: 'pointer' }}>
                {label}
              </button>
            ))}
          </div>
  ...
  // ── Controls Tab with click-to-detail ────────────────────────────────────
  const [controlSearch, setControlSearch] = useState('');
  const [controlSevFilter, setControlSevFilter] = useState('all');
  const [controlFwFilter, setControlFwFilter] = useState('all');

  const filteredControls = useMemo(() => {
    return failingControls.filter(c => {
      const matchSev = controlSevFilter === 'all' || c.severity === controlSevFilter;
      const matchFw  = controlFwFilter  === 'all' || c.framework === controlFwFilter;
      const matchQ   = !controlSearch || (c.title || '').toLowerCase().includes(controlSearch.toLowerCase()) || (c.control_id || '').toLowerCase().includes(controlSearch.toLowerCase());
      return matchSev && matchFw && matchQ;
    });
  }, [failingControls, controlSevFilter, controlFwFilter, controlSearch]);

  const uniqueFrameworks = useMemo(() => [...new Set(failingControls.map(c => c.framework).filter(Boolean))], [failingControls]);
    ...
    <div style={{ position: 'relative' }}>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 14, flexWrap: 'wrap' }}>
        <input
          ...
          style={{ flex: '1 1 200px', padding: '7px 12px', borderRadius: 8, border: '1px solid var(--border-primary)', backgroundColor: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: 13, outline: 'none' }}
        />
        <select value={controlSevFilter} onChange={e => setControlSevFilter(e.target.value)}
          style={{ padding: '7px 10px', borderRadius: 8, border: '1px solid var(--border-primary)', backgroundColor: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: 12, cursor: 'pointer' }}>
          <option value="all">All Severities</option>
          {['critical','high','medium','low'].map(s => <option key={s} value={s}>{s.charAt(0).toUpperCase()+s.slice(1)}</option>)}
        </select>
        <select value={controlFwFilter} onChange={e => setControlFwFilter(e.target.value)}
          style={{ padding: '7px 10px', borderRadius: 8, border: '1px solid var(--border-primary)', backgroundColor: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: 12, cursor: 'pointer' }}>
          <option value="all">All Frameworks</option>
          {uniqueFrameworks.map(fw => <option key={fw} value={fw}>{fw}</option>)}
        </select>
        <span style={{ fontSize: 12, color: 'var(--text-muted)', marginLeft: 'auto' }}>{filteredControls.length} results</span>
      </div>

      {/* Table */}
      <div style={{ borderRadius: 12, border: '1px solid var(--border-primary)', overflow: 'hidden', backgroundColor: 'var(--bg-card)' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border-primary)', backgroundColor: 'var(--bg-secondary)' }}>
              {['Control ID', 'Control', 'Framework', 'Account', 'Severity', 'Days Open', ''].map(h => (
                <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.6, whiteSpace: 'nowrap' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filteredControls.length === 0 ? (
              <tr><td colSpan={7} style={{ padding: '32px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>No failing controls match the current filters</td></tr>
            ) : filteredControls.map((c, i) => {
              const sc = SEV_CFG[c.severity] || SEV_CFG.medium;
              return (
                <tr key={i} onClick={() => setSelectedControl(c)} style={{ borderBottom: '1px solid var(--border-primary)', cursor: 'pointer', transition: 'background 0.12s' }}
                  onMouseEnter={e => e.currentTarget.style.backgroundColor = 'var(--bg-secondary)'}
                  onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                  <td style={{ padding: '10px 14px' }}>
                    <code style={{ fontSize: 11, padding: '2px 6px', borderRadius: 5, backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-tertiary)' }}>{c.control_id}</code>
                  </td>
                  <td style={{ padding: '10px 14px', maxWidth: 280 }}>
                    <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.title}</span>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 5, backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-secondary)' }}>{c.framework}</span>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{c.account}</span>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, fontWeight: 600, backgroundColor: sc.bg, color: sc.text }}>{c.severity}</span>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: c.days_open > 30 ? C.failed : c.days_open > 14 ? C.high : 'var(--text-tertiary)' }}>{c.days_open}d</span>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
      ...
      {/* Drawer */}
      {selectedControl && <ControlDetailDrawer control={selectedControl} onClose={() => setSelectedControl(null)} />}
    </div>
  );
  ...
  // Controls for the selected framework (filtered from failingControls)
  const frameworkControls = useMemo(() => {
    if (!selectedFramework) return [];
    const fwId = selectedFramework.id || selectedFramework.name || '';
    return failingControls.filter(c =>
      (c.framework || '').toLowerCase() === fwId.toLowerCase()
      || (c.framework || '').toLowerCase().includes(fwId.toLowerCase())
    );
  }, [selectedFramework, failingControls]);

  const fwControlColumns = useMemo(() => [
    { accessorKey: 'control_id', header: 'Control ID', size: 140,
      cell: i => <code className="text-xs px-2 py-0.5 rounded" style={{ backgroundColor: 'var(--bg-tertiary)', color: 'var(--text-primary)' }}>{i.getValue()}</code> },
    { accessorKey: 'title', header: 'Control Name',
    ...
    { accessorKey: 'severity', header: 'Severity', size: 100,
      cell: i => {
        const s = (i.getValue() || 'medium').toLowerCase();
        const cols = { critical: '#ef4444', high: '#f97316', medium: '#eab308', low: '#3b82f6' };
        return <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: `${cols[s] || cols.medium}20`, color: cols[s] || cols.medium }}>{s}</span>;
      }},
    { accessorKey: 'total_failed', header: 'Failed Resources', size: 120,
      cell: i => <span className="text-sm font-semibold" style={{ color: 'var(--accent-danger)' }}>{i.getValue() || 0}</span> },
    { accessorKey: 'days_open', header: 'Days Open', size: 100,
      cell: i => <span className="text-sm" style={{ color: 'var(--text-tertiary)' }}>{i.getValue() || 0}d</span> },
  ], []);
  ...
  const renderFrameworksTab = () => (
    <div className="space-y-4">
            ...
            return (
              <div key={fw.id} onClick={() => router.push(`/compliance/${fw.id}`)}
                ...
                style={{
                  ...
                  borderColor: isSelected ? fCol : 'var(--border-primary)',
                  borderWidth: isSelected ? 2 : 1,
                  boxShadow: isSelected ? `0 0 0 1px ${fCol}40` : 'none',
                }}>
      ...
      {/* ── Framework Controls Detail Panel ── */}
      {selectedFramework && (
        <div className="rounded-xl border p-5" style={{ backgroundColor: 'var(--bg-card)', borderColor: 'var(--border-primary)' }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-base font-bold" style={{ color: 'var(--text-primary)' }}>
                {selectedFramework.name} — Failing Controls
              </h3>
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                {frameworkControls.length} controls failing · {selectedFramework.controls || 0} total controls
              </p>
            </div>
            <button
              onClick={(e) => { e.stopPropagation(); setSelectedFramework(null); }}
              className="text-xs px-3 py-1.5 rounded-lg border"
              style={{ borderColor: 'var(--border-primary)', color: 'var(--text-secondary)', backgroundColor: 'var(--bg-tertiary)' }}
            >
              Close
            </button>
          </div>
          {frameworkControls.length > 0 ? (
            <DataTable
              ...
              columns={fwControlColumns}
              pageSize={15}
              emptyMessage="No failing controls"
            />
          ) : (
            <div className="text-center py-6" style={{ color: 'var(--text-tertiary)' }}>
              No failing controls found for this framework in the current scan data.
              <br />
              <span className="text-xs">Controls may not be mapped yet, or all controls are passing.</span>
            </div>
          )}
        </div>
      )}
  ...
  const renderOverviewTab = () => (
    <>
      ...
      {/* ── Frameworks Section ── */}
      <div style={{ borderTop: '1px solid var(--border-primary)', paddingTop: 24, marginTop: 8 }}>
        <SectionHeader
          title="Frameworks"
          sub="Click any framework to view detailed controls and resource-level findings"
          action={
      ...
      <div style={{ borderTop: '1px solid var(--border-primary)', paddingTop: 24, marginTop: 8 }}>
        <SectionHeader
          ...
          sub="Per-account compliance scores across all frameworks"
        />
        {renderMatrixTab()}
      </div>
    </>
  ...
  const tabData = {
    overview:   { renderTab: renderOverviewTab },
    controls:   { renderTab: renderControlsTab },
