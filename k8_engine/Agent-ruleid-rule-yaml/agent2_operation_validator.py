...
def load_k8s_catalog():
    """Load K8s API catalog"""
...
def find_best_operation(resource_name: str, required_fields: List[str], catalog: Dict) -> Optional[Dict]:
    """
    Find the best K8s operation that provides the required fields.
    
    Args:
        resource_name: K8s resource name
        required_fields: List of field names needed
        catalog: K8s API catalog
    
    Returns:
        Best matching operation or None
    """
    resource_data = catalog.get(resource_name, {})
    
    best_match = None
    best_score = 0
    
    for resource_name, resource_data in resource_data.get('resources', {}).items():
...
def validate_resource_requirements(resource_name: str, requirements: List[Dict], catalog: Dict) -> List[Dict]:
    """Validate and enrich requirements for a resource"""
    validated = []
    ...
    for req in requirements:
        ...
        # Extract field names
        field_names = [f.get('k8s_api_field', '') for f in fields]
        
        # Find best operation
        best_op = find_best_operation(resource_name, field_names, catalog)
...
def main():
    logger.info("Agent 2: K8s Operation Validator starting")
    print("=" * 80)
    print("AGENT 2: K8s Operation Validator")
    print("=" * 80)
    print()
    
    # Load K8s catalog
    print("Loading K8s API catalog...")
    catalog = load_k8s_catalog()
    print(f"✅ Loaded {len(catalog)} resources")
    print()
    ...
    for resource_name, requirements in all_requirements.items():
        print(f"\n📦 {resource_name}")
        logger.info(f"Validating resource: {resource_name}")
        
        validated = validate_resource_requirements(resource_name, requirements, catalog)
        all_validated[resource_name] = validated
        ...
        logger.info(f"Service {resource_name}: {pass_count} passed, {fail_count} failed")
