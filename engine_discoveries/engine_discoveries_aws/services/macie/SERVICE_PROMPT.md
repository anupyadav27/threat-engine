# MACIE Service Processing Prompt

## Task
Process MACIE service metadata mapping and YAML generation following this sequence:

1. **Review all metadata from MACIE**
   - Review all metadata files in `/Users/apple/Desktop/threat-engine/aws_compliance_python_engine/services/macie/metadata/`
   - Understand security requirements and intent

2. **Create metadata_mapping.json**
   - Use boto3 database files from `/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/macie/`
   - Reference:
...
3. **Review as AWS Python developer and security expert**
   - Verify methods exist in boto3 MACIE client
...
4. **Generate YAML from metadata_mapping.json**
   - Run: `cd /Users/apple/Desktop/threat-engine/aws_compliance_python_engine && python3 generate_yaml_from_metadata_mapping.py macie`
   - Verify YAML file is created in `services/macie/rules/macie.yaml`
...
5. **MANDATORY: Test with actual AWS resources**
   ...
   **Step 5a: Check for existing resources**
   ...
   client = session.client('macie', region_name='us-east-1')
   ...
   **Option B: Create test resources (if none exist)**
   - **REQUIRED**: Create a test script: `test_macie_resources.py`
   ...
   - The test script MUST:
     ...
     2. **Run compliance scan** automatically:
        ...
        subprocess.run([
            ...
            '--service', 'macie', '--region', 'us-east-1'
        ])
        ```
   ...
   - Run the test script:
     ```bash
     cd /Users/apple/Desktop/threat-engine/aws_compliance_python_engine
     python3 test_macie_resources.py
     ```
   ...
   **Step 5c: Run compliance scan**
   ...
   python3 -m aws_compliance_python_engine.engine.main_scanner --service macie --region us-east-1
   ```
   ...
   **Step 5e: Analyze scan results**
   ...
   find $LATEST_SCAN -name "*.json" -path "*/macie/*" | head -5 | xargs cat
   ```
...
6. **Detailed output analysis (MANDATORY)**
   ...
   **6c: Verify field paths in check evidence**
   ...
   find $LATEST_SCAN -name "*.json" -path "*/macie/*" | head -3 | while read file; do
       echo "Checking: $file"
...
7. **Fix issues and retest (iterative process)**
   ...
   **7a: Fix identified issues**
   ...
   - If discovery errors: Fix `rules/macie.yaml` directly or fix `metadata_mapping.json`
   ...
   **7b: Regenerate YAML (if metadata_mapping.json changed)**
   ```bash
   cd /Users/apple/Desktop/threat-engine/aws_compliance_python_engine
   python3 generate_yaml_from_metadata_mapping.py macie
   ```
