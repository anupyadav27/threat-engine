#!/usr/bin/env python3
"""
Workspaces Metadata Review Analysis Script
Analyzes workspaces service metadata for consolidation and cross-service placement opportunities.
"""
...
BOTO3_DEPS_FILE = Path("/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/workspaces/boto3_dependencies_with_python_names_fully_enriched.json")
...
def check_method_ownership(method: str) -> Tuple[Optional[str], int]:
    ...
    try:
        ...
        # Check if method exists in workspaces service
        workspaces_data = boto3_data.get("workspaces", {})
        workspaces_ops = workspaces_data.get("independent", []) + workspaces_data.get("dependent", [])
        workspaces_has_method = any(
            op.get("python_method") == method 
            for op in workspaces_ops
        )
        ...
        for service_dir in boto3_base.iterdir():
            if not service_dir.is_dir() or service_dir.name == "workspaces":
                continue
        ...
        if workspaces_has_method:
            if service_count > 0:
                return "workspaces", service_count + 1  # Ambiguous
            return "workspaces", 1  # Only in workspaces
        elif service_count > 0:
            return first_other_service, service_count  # Not in workspaces, in other service(s)
        else:
...
def extract_service_from_rule_id(rule_id: str) -> str:
    """Extract service name from rule_id (e.g., aws.workspaces.workspace.x -> workspaces)"""
    parts = rule_id.split(".")
...
def main():
    ...
    # Try different possible keys for the rules array
    rules = mapping_data.get("workspaces_metadata_mapping", [])
    if not rules:
        rules = mapping_data.get("metadata_mapping", [])
    if not rules and isinstance(mapping_data, list):
        rules = mapping_data
