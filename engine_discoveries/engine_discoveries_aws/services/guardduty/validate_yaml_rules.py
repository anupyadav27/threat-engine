#!/usr/bin/env python3
"""
Validate GUARDDUTY YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "DetectorIds" with items_for -> "item.DetectorId"
    - field_path: "DataSources.S3Logs.Enable" -> "item.DataSources.S3Logs.Enable"
    """
    ...
    if uses_items_for:
        ...
        if clean_path == 'DetectorIds':
            # Special case: list_detectors emits DetectorId from DetectorIds array
            return 'item.DetectorId'
        return f"item.{clean_path}" if clean_path else "item"
    else:
        # If not items_for, check if emit extracts from a specific path
        # Remove common prefixes
        if clean_path.startswith('DetectorIds'):
            clean_path = clean_path.replace('DetectorIds', 'DetectorId')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('guardduty_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'guardduty.yaml'
