#!/usr/bin/env python3
"""
Validate QUICKSIGHT YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "DataSetSummaries[].DataSetId" with items_for -> "item.DataSetId"
    - field_path: "AccountSettings.PublicSharingEnabled" -> "item.PublicSharingEnabled"
    - field_path: "Permissions[].Principal" -> "item.Permissions[].Principal"
    """
    # Check if emit uses items_for (array iteration)
    uses_items_for = 'items_for' in emit_structure
    
    # Handle array notation - preserve it
    has_array_notation = '[].' in field_path
    
    if uses_items_for:
        # If items_for, each item is already 'item', so remove array prefix if present
        # But preserve nested array notation like Permissions[]
        if field_path.startswith('DataSetSummaries'):
            clean_path = field_path.replace('DataSetSummaries.', '')
        elif field_path.startswith('UserList.'):
            clean_path = field_path.replace('UserList.', '')
        elif field_path.startswith('GroupList.'):
            clean_path = field_path.replace('GroupList.', '')
        elif field_path.startswith('DashboardSummaryList.'):
            clean_path = field_path.replace('DashboardSummaryList.', '')
        else:
        ...
        # For describe_account_settings, the emit shows AccountSettings fields directly
        # but YAML may use item.AccountSettings.*, so keep the full path
        if field_path.startswith('AccountSettings.'):
            # Keep AccountSettings prefix if YAML uses it
            clean_path = field_path
        else:
            clean_path = field_path
        return f"item.{clean_path}" if clean_path else "item"
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('quicksight_metadata_mapping', []):
    ...
    if discovery_id and expected_method:
        ...
        if discovery:
            ...
            if calls:
                ...
                # Handle special case for STS client
                if calls[0].get('client') == 'sts' and actual_method == 'get_caller_identity':
                    actual_method = 'get_caller_identity'
    ...
    for i, expected_field in enumerate(expected_fields):
        ...
        # Handle array notation - if expected path has array notation, keep it in normalized path
        if '[].' in expected_path:
            # Keep array notation in normalized path
            if '[].' not in normalized_path:
                # Add array notation back
                field_name = expected_path.split('[].')[-1]
                parent = expected_path.split('[].')[0]
                if parent in normalized_path:
                    normalized_path = normalized_path.replace(parent, f'{parent}[]')
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'quicksight.yaml'
