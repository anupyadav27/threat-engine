"""
Generate YAML discovery files for IBM Cloud services from minimal_operations_list.json.
...
def operation_to_action(operation: str) -> str:
    """Convert IBM operation name to action name.
    IBM operations are like 'list_users' -> 'list-users'
    """
    # Convert snake_case to kebab-case (with hyphens)
    return operation.replace('_', '-')

def entity_to_field_name(entity: str) -> str:
    """Convert entity name to field name for params."""
    # ibm.iam.account_setting.id -> account_setting.id (keep as is for IBM)
    # Or extract just the field part
    parts = entity.split('.')
    if len(parts) > 2:
...
def find_operation_producing_entity(entity: str, operations: List[Dict], service: str) -> Optional[str]:
    ...
    for op_info in operations:
        ...
        if entity in entities_covered:
            ...
            return f"ibm.{service}.{action}"
    return None

def get_ibm_info(operation: str, ibm_data: Dict, service_name: str) -> Optional[Dict]:
    """Get IBM action and main_output_field for an operation."""
    if not ibm_data:
        return None
    ...
    for op_data in all_operations:
        if op_data.get("operation") == operation:
            return {
                "action": op_data.get("yaml_action") or op_data.get("operation", operation),
...
def generate_discovery_entries(service_name: str, service_dir: Path) -> List[Dict]:
    ...
    for op_info in operations:
        ...
        # Get IBM info
        ibm_info = get_ibm_info(operation, ibm_data, service_name)
        if not ibm_info:
            ...
            action = ibm_info["action"]
            main_output = ibm_info.get("main_output_field")
            required_params = ibm_info.get("required_params", [])
        
        # Build discovery_id
        # Format: ibm.service.action
        discovery_id = f"ibm.{service_name}.{action}"
...
def get_ibm_module(service_name: str, service_dir: Path) -> str:
    """Get IBM module path for service."""
    ibm_file = service_dir / "ibm_dependencies_with_python_names_fully_enriched.json"
    if ibm_file.exists():
        try:
            with open(ibm_file, 'r') as f:
            ...
            # IBM uses service_class or package from service_data
            ...
            pass
    # Default IBM Cloud SDK pattern
    return f"ibm_cloud_sdk.{service_name}"
...
def generate_yaml_discovery_file(service_name: str, service_dir: Path, output_file: Path) -> bool:
    ...
    # Get IBM module
    ibm_module = get_ibm_module(service_name, service_dir)
...
def generate_all_services(base_dir: Path):
    ...
    print("="*80)
    print("GENERATING YAML DISCOVERY FILES FOR ALL IBM SERVICES")
    print("="*80)
    
    # Get all service directories - include those with minimal_operations_list OR dependency_index/ibm_dependencies
