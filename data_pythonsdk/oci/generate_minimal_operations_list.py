...
def is_read_operation_by_name(op_name: str) -> bool:
    ...
    Based on OCI patterns:
    - list operations: list_*
    - get operations: get_*
    """
    if not op_name:
        return False
...
def build_fallback_dependency_index_from_oci(service_name: str, service_dir: Path) -> Optional[Dict]:
    """Build a minimal dependency_index from oci_dependencies when dependency_index is empty.
    Includes operations that:
    1. Have item_fields (preferred), OR
    2. Have names that suggest read operations (list_*, get_*)
    """
    
    oci_file = service_dir / "oci_dependencies_with_python_names_fully_enriched.json"
    if not oci_file.exists():
        return None
    ...
    for op_data in all_operations:
        ...
        is_independent = op_name.lower().startswith('list_')
        
        # Extract resource name from operation for entity naming
        # e.g., list_compartments -> compartments, get_compartment -> compartment
        ...
        # Create entity name: oci.service.resource_name
        entity_name = f"oci.{service_name}.{resource_name}"
...
def generate_operations_report(service_name: str, service_dir: Path) -> Optional[Dict]:
    ...
    # If dependency_index is empty, try to build from oci_dependencies
    if dependency_index is None:
        dependency_index = build_fallback_dependency_index_from_oci(service_name, service_dir)
        if dependency_index is None:
...
def generate_all_services(base_dir: Path):
    ...
    print("="*80)
    print("GENERATING MINIMAL OPERATIONS LIST FOR ALL OCI SERVICES")
    print("="*80)
    
    # Get all service directories - include those with dependency_index OR oci_dependencies
