#!/usr/bin/env python3
"""
Generate field_operator_value_table.csv for GCP services.
Adapted from Azure version for GCP structure.
"""
...
def load_service_data(service_name: str, base_dir: Path) -> Dict[str, Any]:
    ...
    if direct_vars_path.exists():
        with open(direct_vars_path, 'r', encoding='utf-8') as f:
            ...
            # Collect all fields from direct_vars
            fields = data['direct_vars'].get('fields', {})
            data['all_fields'].update(fields.keys())
    
    # Load gcp_dependencies_with_python_names_fully_enriched.json
    gcp_deps_path = service_dir / 'gcp_dependencies_with_python_names_fully_enriched.json'
    if gcp_deps_path.exists():
        with open(gcp_deps_path, 'r', encoding='utf-8') as f:
            gcp_data = json.load(f)
            data['gcp_deps'] = gcp_data.get(service_name, {})
            
            # Collect all fields from gcp_deps
            resources = data['gcp_deps'].get('resources', {})
                ...
                for op_type in ['independent', 'dependent']:
                    for op in resource_data.get(op_type, []):
                        ...
                        if isinstance(item_fields, dict):
                            data['all_fields'].update(item_fields.keys())
...
def get_possible_values(field_name: str, service_data: Dict) -> tuple:
    ...
    # Check gcp_deps
    if service_data['gcp_deps']:
        resources = service_data['gcp_deps'].get('resources', {})
        for resource_name, resource_data in resources.items():
            for op_type in ['independent', 'dependent']:
                for op in resource_data.get(op_type, []):
                    item_fields = op.get('item_fields', {})
                    if isinstance(item_fields, dict) and field_name in item_fields:
                        ...
                        if field_data.get('enum') and 'possible_values' in field_data:
                            return (field_data['possible_values'], 'gcp_deps')
...
def get_operators(field_name: str, service_data: Dict) -> List[str]:
    ...
    if not operators:
        ...
        if field_type in ['integer', 'long', 'float', 'double', 'number']:
        ...
        elif field_type == 'timestamp' or 'timestamp' in field_type.lower():
            return ['equals', 'not_equals', 'greater_than', 'less_than', 
                   'greater_than_or_equal', 'less_than_or_equal', 'exists']
        else:
        ...
        for resource_name, resource_data in resources.items():
            for op_type in ['independent', 'dependent']:
                for op in resource_data.get(op_type, []):
                    item_fields = op.get('item_fields', {})
                    if isinstance(item_fields, dict) and field_name in item_fields:
                        if item_fields[field_name].get('enum'):
                            return True
    
    # Check direct_vars for enum flag
    if service_data['direct_vars']:
        fields = service_data['direct_vars'].get('fields', {})
        if field_name in fields:
            field_data = fields[field_name]
            if isinstance(field_data, dict) and field_data.get('enum'):
                return True
...
def generate_all_services(base_dir: Path):
    ...
    print("="*80)
    print("GENERATING FIELD-OPERATOR-VALUE TABLES FOR ALL GCP SERVICES")
    print("="*80)
