...
Two phases:
  1. STORE:  Accept in-memory actor stats from the current scan (aggregated during
             the read loop in run_scan.py) and persist to cdr_actor_daily_stats.
  ...
  2. DETECT: Compare today's in-memory stats against the historical window stored
             in cdr_actor_daily_stats → flag anomalies → write to cdr_findings.

cdr_actor_daily_stats schema:
...
CREATE TABLE IF NOT EXISTS cdr_actor_daily_stats (
...
CREATE INDEX IF NOT EXISTS idx_cads_tenant_date
    ON cdr_actor_daily_stats (tenant_id, stat_date);
CREATE INDEX IF NOT EXISTS idx_cads_entity
    ON cdr_actor_daily_stats (tenant_id, entity_type, entity_key);
"""

CREATE_BASELINES_DDL = """
CREATE TABLE IF NOT EXISTS cdr_baselines (
...
class BaselineEvaluator:
    ...
    def store_daily_stats(
        ...
        """Persist today's in-memory aggregated stats to cdr_actor_daily_stats.
        ...
        try:
            with conn.cursor() as cur:
                execute_values(cur, """
                    INSERT INTO cdr_actor_daily_stats
                        (tenant_id, account_id, entity_type, entity_key, metric_name, stat_date, value)
                    VALUES %s
    ...
    def _load_historical(
        ...
        """Load daily stat rows from cdr_actor_daily_stats for the historical window.
