...
Writes:
  - cdr_findings  (matched events)
  - cdr_report    (scan summary)
"""
...
class CIEMRuleEvaluator:
    ...
    def _get_cdr_conn(self):
        return psycopg2.connect(
            ...
            password=self._pw("CDR_DB_PASSWORD", "INVENTORY_DB_PASSWORD", "DB_PASSWORD"),
        )
    ...
    def _write_findings(self, findings: List[Dict]):
        ...
        try:
            ...
            conn.commit()
            logger.info(f"L1: wrote {len(findings)} findings to cdr_findings")
        finally:
            conn.close()
