...
class AWSCIEMProvider(BaseCIEMProvider):
    def get_parsers(self) -> Dict[str, Type]:
        from cdr_engine.parser.cloudtrail_parser import CloudTrailParser
        from cdr_engine.parser.vpc_flow_parser import VPCFlowParser
        from cdr_engine.parser.alb_parser import ALBParser
        from cdr_engine.parser.waf_parser import WAFParser
        from cdr_engine.parser.cloudfront_parser import CloudFrontParser
        from cdr_engine.parser.s3_access_parser import S3AccessParser
        from cdr_engine.parser.dns_parser import DNSParser
        from cdr_engine.parser.rds_audit_parser import RDSAuditParser
        from cdr_engine.parser.eks_audit_parser import EKSAuditParser
        from cdr_engine.parser.lambda_parser import LambdaParser
        from cdr_engine.parser.guardduty_parser import GuardDutyParser
