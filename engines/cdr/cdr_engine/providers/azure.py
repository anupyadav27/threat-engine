...
class AzureCIEMProvider(BaseCIEMProvider):
    def get_parsers(self) -> Dict[str, Type]:
        extra: Dict[str, Type] = {}
        try:
            from cdr_engine.parser.azure_activity_parser import AzureActivityParser
            from cdr_engine.parser.azure_nsg_flow_parser import AzureNSGFlowParser
    ...
    def get_readers(self) -> Dict[str, Type]:
        extra: Dict[str, Type] = {}
        try:
            from cdr_engine.reader.azure_blob_reader import AzureBlobReader
            extra["azure_blob"] = AzureBlobReader
        except ImportError:
            pass
        try:
            from cdr_engine.reader.azure_monitor_reader import AzureMonitorReader
