...
Endpoints:
  ...
  GET  /api/v1/cdr/findings              — Query CIEM detection findings
  GET  /api/v1/cdr/findings/{finding_id} — Get single finding detail
  GET  /api/v1/cdr/dashboard             — Dashboard summary (counts, trends)
  GET  /api/v1/cdr/identities            — Identity risk summary
  GET  /api/v1/cdr/top-rules             — Top triggered rules
  GET  /api/v1/cdr/log-sources           — Log source coverage status
  GET  /api/v1/cdr/report/{scan_run_id}  — Scan report
...
from engine_common.db_connections import get_cdr_conn, get_onboarding_conn
...
logger = setup_logger(__name__, engine_name="engine-cdr")
...
configure_telemetry("engine-cdr", app)
...
# Scanner job config
SCANNER_IMAGE = os.getenv("CDR_SCANNER_IMAGE", "yadavanup84/engine-cdr:v-std-cols")
...
async def start_all_accounts_cdr(
...
async def start_all_accounts_cdr_internal():
    ...
    it is cluster-internal only (engine-cdr service, port 80).
...
async def query_findings(
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            cur.execute(f"SELECT count(*) FROM cdr_findings WHERE {where}", params)
...
@app.get("/api/v1/cdr/findings/{finding_id}")
async def get_finding(finding_id: str):
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT * FROM cdr_findings WHERE finding_id = %s", (finding_id,))
            row = cur.fetchone()
            if not row:
...
@app.get("/api/v1/cdr/findings/{finding_id}/timeline")
...
def _query_cdr_scan_trend(cur, tenant_id: str) -> list:
    ...
    try:
        ...
        logger.warning("cdr scan_trend query failed", exc_info=True)
        return []


@app.get("/api/v1/cdr/dashboard")
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            cur.execute(f"""
                ...
                FROM cdr_findings WHERE tenant_id = %s {scan_filter}
            ...
            scan_trend = _query_cdr_scan_trend(cur, tenant_id)
...
@app.get("/api/v1/cdr/identities/heatmap")
...
@app.get("/api/v1/cdr/log-sources")
async def log_sources(
    auth: Any = Depends(require_permission("cdr:read")),
) -> Dict[str, Any]:
    """Return log source coverage status for the CIEM logSources tab.

    Aggregates cdr_findings by source_type and account, deriving an
...
@app.get("/api/v1/cdr/report/{scan_run_id}")
async def scan_report(scan_run_id: str):
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT * FROM cdr_report WHERE scan_run_id = %s", (scan_run_id,))
            report = cur.fetchone()
            if not report:
...
class CiemFindingsByResourceResponse(BaseModel):
    """Response for GET /api/v1/cdr/findings/by-resource."""
...
_CIEM_TABLE = "cdr_findings"
_CIEM_TITLE_EXPR = "COALESCE(title, rule_id)"


@app.get("/api/v1/cdr/findings/by-resource", response_model=CiemFindingsByResourceResponse)
async def get_cdr_findings_by_resource(
    ...
    try:
        ...
        logger.error(f"Error in get_cdr_findings_by_resource: {exc}", exc_info=True)
