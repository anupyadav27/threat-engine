"""
Database Configuration Management for Threat Engine
...
def get_database_config(engine_name: str = "check") -> DatabaseConnectionConfig:
    """Get database configuration. 'check' = read from Check DB; 'threat' = write reports to Threat DB."""
    ...
    if engine_name == "threat":
        return DatabaseConnectionConfig(
            ...
            username=os.getenv("THREAT_DB_USER", "threat_user"),
    ...
    raise ValueError(f"Threat engine supports 'check' or 'threat', got: {engine_name}")


def get_connection_string(engine_name: str = "check") -> str:
    """Get connection string for check or threat database."""
