"""
middleware category scanner — stub.
...
class MiddlewareScanner(TechScanner):
    """Scanner for category: middleware"""

    async def connect(self) -> None:
        """TODO Sprint: implement connection for middleware tech_type={self.tech_type}"""
        logger.info(f"[middleware] connect host={self.host} tech_type={self.tech_type}")

    async def discover(self) -> List[TechFinding]:
        """Load YAML queries and execute them against the connected middleware target."""
    ...
    async def disconnect(self) -> None:
        logger.info(f"[middleware] disconnect host={self.host}")
