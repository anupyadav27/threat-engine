...
class DevOpsScanner(TechScanner):
    """Scanner for category: devops"""

    async def connect(self) -> None:
        """TODO Sprint: implement connection for devops tech_type={self.tech_type}"""
        logger.info(f"[devops] connect host={self.host} tech_type={self.tech_type}")

    async def discover(self) -> List[TechFinding]:
        """Load YAML queries and execute them against the connected devops target."""
    ...
    async def disconnect(self) -> None:
        logger.info(f"[devops] disconnect host={self.host}")
