...
class NetworkScanner(TechScanner):
    """Scanner for category: network"""

    async def connect(self) -> None:
        """TODO Sprint: implement connection for network tech_type={self.tech_type}"""
        logger.info(f"[network] connect host={self.host} tech_type={self.tech_type}")

    async def discover(self) -> List[TechFinding]:
        """Load YAML queries and execute them against the connected network target."""
    ...
    async def disconnect(self) -> None:
        logger.info(f"[network] disconnect host={self.host}")
