"""
data_platform category scanner — stub.
...
class DataPlatformScanner(TechScanner):
    """Scanner for category: data_platform"""

    async def connect(self) -> None:
        """TODO Sprint: implement connection for data_platform tech_type={self.tech_type}"""
        logger.info(f"[data_platform] connect host={self.host} tech_type={self.tech_type}")

    async def discover(self) -> List[TechFinding]:
        """Load YAML queries and execute them against the connected data_platform target."""
    ...
    async def disconnect(self) -> None:
        logger.info(f"[data_platform] disconnect host={self.host}")
