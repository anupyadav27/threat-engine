...
class LinuxScanner(TechScanner):
    """Scanner for category: linux"""

    async def connect(self) -> None:
        """TODO Sprint: implement connection for linux tech_type={self.tech_type}"""
        logger.info(f"[linux] connect host={self.host} tech_type={self.tech_type}")

    async def discover(self) -> List[TechFinding]:
        """Load YAML queries and execute them against the connected linux target."""
    ...
    async def disconnect(self) -> None:
        logger.info(f"[linux] disconnect host={self.host}")
