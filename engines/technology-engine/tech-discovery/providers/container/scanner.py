"""
container category scanner — stub.
...
class ContainerScanner(TechScanner):
    """Scanner for category: container"""

    async def connect(self) -> None:
        """TODO Sprint: implement connection for container tech_type={self.tech_type}"""
        logger.info(f"[container] connect host={self.host} tech_type={self.tech_type}")

    async def discover(self) -> List[TechFinding]:
        """Load YAML queries and execute them against the connected container target."""
    ...
    async def disconnect(self) -> None:
        logger.info(f"[container] disconnect host={self.host}")
