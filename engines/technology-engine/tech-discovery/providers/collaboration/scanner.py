"""
collaboration category scanner — stub.
...
class CollaborationScanner(TechScanner):
    """Scanner for category: collaboration"""

    async def connect(self) -> None:
        """TODO Sprint: implement connection for collaboration tech_type={self.tech_type}"""
        logger.info(f"[collaboration] connect host={self.host} tech_type={self.tech_type}")

    async def discover(self) -> List[TechFinding]:
        """Load YAML queries and execute them against the connected collaboration target."""
    ...
    async def disconnect(self) -> None:
        logger.info(f"[collaboration] disconnect host={self.host}")
