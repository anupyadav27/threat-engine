"""
web_server category scanner — stub.
...
class WebServerScanner(TechScanner):
    """Scanner for category: web_server"""

    async def connect(self) -> None:
        """TODO Sprint: implement connection for web_server tech_type={self.tech_type}"""
        logger.info(f"[web_server] connect host={self.host} tech_type={self.tech_type}")

    async def discover(self) -> List[TechFinding]:
        """Load YAML queries and execute them against the connected web_server target."""
    ...
    async def disconnect(self) -> None:
        logger.info(f"[web_server] disconnect host={self.host}")
