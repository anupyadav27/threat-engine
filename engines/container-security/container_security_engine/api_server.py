...
class FindingItem(BaseModel):
    """Single container security finding row returned by the asset-context endpoint."""
...
class FindingsByResourceResponse(BaseModel):
    """Response for GET /api/v1/container-security/findings/by-resource."""
...
_CSEC_TABLE = "container_sec_findings"


def _get_csec_conn():
    """Return psycopg2 connection to the container security database.

    Returns:
        Active psycopg2 connection using CSEC_DB_* env vars.
    """
    import psycopg2
...
@app.get("/api/v1/container-security/findings/by-resource", response_model=FindingsByResourceResponse)
async def get_container_findings_by_resource(
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(f"""
                SELECT scan_run_id FROM {_CSEC_TABLE}
        ...
        logger.error(f"Error in get_container_findings_by_resource: {exc}", exc_info=True)
