#!/usr/bin/env python3
"""
Validate SES YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "Rules[].Name" with items_for -> "item.Name"
    - field_path: "ConfigurationSets[].Name" -> "item.Name"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('Rules.'):
            clean_path = clean_path.replace('Rules.', '')
        elif clean_path.startswith('ConfigurationSets.'):
            clean_path = clean_path.replace('ConfigurationSets.', '')
        elif clean_path.startswith('VerifiedEmailAddresses.'):
            clean_path = clean_path.replace('VerifiedEmailAddresses.', '')
        elif clean_path.startswith('PolicyNames.'):
            clean_path = clean_path.replace('PolicyNames.', '')
        # For list_identity_policies, emit uses PolicyName (singular) not PolicyNames
        # The emit structure shows: PolicyName: '{{ policy_name }}'
        # So PolicyNames[] should normalize to item.PolicyName
        if clean_path == '' and 'PolicyNames' in field_path:
            clean_path = 'PolicyName'
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('ses_metadata_mapping', []):
    ...
    # Handle case where rule has no conditions key at all
    ...
    if not conditions or len(conditions) == 0:
        # Rule has no conditions - just checks if resources exist
        condition_list = []
    ...
    # Handle case where rule has no conditions (just checks if resources exist)
    if len(condition_list) == 0 and len(expected_fields) == 0:
        # This is valid - rule just checks if discovery returns any items
        pass
    elif len(condition_list) != len(expected_fields):
    ...
    for i, expected_field in enumerate(expected_fields):
        ...
        # Handle empty field path - means check if item exists
        if expected_path == '':
        ...
        if expected_value is not None:
            ...
            if isinstance(expected_value, list) and isinstance(actual_value, list):
                # Check if lists contain hashable types
                try:
                    ...
                    # Lists contain unhashable types (dicts, lists), compare as lists
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'ses.yaml'
