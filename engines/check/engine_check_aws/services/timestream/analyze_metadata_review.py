#!/usr/bin/env python3
"""
Timestream Metadata Review Analysis Script
Analyzes timestream service metadata for consolidation and cross-service placement opportunities.
"""
...
BOTO3_DEPS_FILE = Path("/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/timestream/boto3_dependencies_with_python_names_fully_enriched.json")
...
def check_method_ownership(method: str) -> Tuple[Optional[str], int]:
    ...
    try:
        ...
        # Check if method exists in timestream service
        timestream_ops = boto3_data.get("timestream", {}).get("operations", [])
        timestream_has_method = any(
            op.get("python_method") == method 
            for op in timestream_ops
        )
        
        # Count how many services have this method
        service_count = 0
        for service_name, service_data in boto3_data.items():
            if service_name == "timestream":
                continue
        ...
        if timestream_has_method:
            if service_count > 0:
                return "timestream", service_count + 1  # Ambiguous
            return "timestream", 1  # Only in timestream
        elif service_count > 0:
            return first_other_service, service_count  # Not in timestream, in other service(s)
        else:
...
def extract_service_from_rule_id(rule_id: str) -> str:
    """Extract service name from rule_id (e.g., aws.timestream.database.x -> timestream)"""
    parts = rule_id.split(".")
...
def analyze_cross_service(mapping_data: List[Dict], metadata: Dict[str, Dict]) -> List[Dict]:
    ...
    for rule in mapping_data:
        ...
        if method_service and method_service != "timestream":
        ...
        elif method_service == "timestream" and service_count > 1:
            # Method exists in timestream but also in other services (ambiguous)
            suggestions.append({
                "rule_id": rule_id,
                "metadata_file": f"{rule_id}.yaml",
                "current_service": "timestream",
                "suggested_service": "timestream",
                "reason": f"Method {method} exists in timestream but also in {service_count-1} other service(s) - verify correct usage",
                "python_method": method,
...
def main():
    ...
    with open(METADATA_MAPPING_FILE, 'r') as f:
        mapping_data = json.load(f)["timestream_metadata_mapping"]
    ...
    cross_service_count = len([s for s in cross_service if s.get("suggested_service") != "timestream"])
    ...
    if cross_service_count > 0:
        cross_service_confidence = sum(
            ...
            if s.get("suggested_service") != "timestream"
        ) / cross_service_count
    else:
