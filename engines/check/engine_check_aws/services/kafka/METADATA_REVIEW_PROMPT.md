# kafka Metadata Review and Consolidation Prompt

You are an expert AWS compliance engineer reviewing metadata mappings and metadata files for the **kafka** service. Your task is to analyze and provide actionable suggestions for consolidations and cross-service placements.

## Service Context

**Service Name**: `kafka`  
**Service Path**: `/Users/apple/Desktop/threat-engine/aws_compliance_python_engine/services/kafka/`
...
**Path**: `/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/kafka/boto3_dependencies_with_python_names_fully_enriched.json`

Contains actual AWS API method specifications to validate method ownership.

## Your Task

Analyze the kafka service and provide:
...
{{
  ...
  "consolidation_suggestions": {{
    "duplicates": [
      {{
        "keep": {{
          "rule_id": "aws.kafka.resource.check_complete",
          "metadata_file": "aws.kafka.resource.check_complete.yaml",
        ...
        "remove": [
          {{
            "rule_id": "aws.kafka.resource.check_basic",
            "metadata_file": "aws.kafka.resource.check_basic.yaml",
            "replaced_by": "aws.kafka.resource.check_complete",
  ...
  "cross_service_suggestions": [
    {{
      "rule_id": "aws.kafka.resource.check_name",
      "metadata_file": "aws.kafka.resource.check_name.yaml",
...
2. **Service Prefix Matching**
   - Extract service from rule_id: `aws.kafka.{{resource}}.{{check}}`
...
1. **Load all files** for kafka service
...
**Begin Review**: Analyze the kafka service files and generate comprehensive review JSON.
