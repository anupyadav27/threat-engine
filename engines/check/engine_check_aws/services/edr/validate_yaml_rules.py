#!/usr/bin/env python3
"""
Validate EDR YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "items[].membershipStatus" with items_for -> "item.membershipStatus"
    """
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('edr_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'edr.yaml'
