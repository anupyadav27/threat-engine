#!/usr/bin/env python3
"""
Validate SERVICECATALOG YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "PortfolioDetails[].Id" with items_for -> "item.Id"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('PortfolioDetails.'):
            clean_path = clean_path.replace('PortfolioDetails.', '')
        elif clean_path.startswith('PortfolioShareDetails.'):
            clean_path = clean_path.replace('PortfolioShareDetails.', '')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('servicecatalog_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'servicecatalog.yaml'
