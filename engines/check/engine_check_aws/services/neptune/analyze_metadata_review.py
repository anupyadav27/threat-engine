#!/usr/bin/env python3
"""
Neptune Metadata Review Analysis Script
Analyzes neptune service metadata for consolidation and cross-service placement opportunities.
"""
...
BOTO3_DEPS_FILE = Path("/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/neptune/boto3_dependencies_with_python_names_fully_enriched.json")
...
def check_method_ownership(method: str) -> Tuple[Optional[str], int]:
    ...
    try:
        ...
        # Check if method exists in neptune service
        neptune_ops = boto3_data.get("neptune", {}).get("operations", [])
        neptune_has_method = any(
            op.get("python_method") == method 
            for op in neptune_ops
        )
        ...
        if neptune_has_method:
            if service_count > 0:
                return "neptune", service_count + 1  # Ambiguous
            return "neptune", 1  # Only in neptune
        elif service_count > 0:
            return first_other_service, service_count  # Not in neptune, in other service(s)
        else:
...
def extract_service_from_rule_id(rule_id: str) -> str:
    """Extract service name from rule_id (e.g., aws.neptune.cluster.x -> neptune)"""
    parts = rule_id.split(".")
...
def analyze_cross_service(mapping_data: List[Dict], metadata: Dict[str, Dict]) -> List[Dict]:
    ...
    for rule in mapping_data:
        ...
        if method_service and method_service != "neptune":
        ...
        elif method_service == "neptune" and service_count > 1:
            # Method exists in neptune but also in other services (ambiguous)
            suggestions.append({
                "rule_id": rule_id,
                "metadata_file": f"{rule_id}.yaml",
                "current_service": "neptune",
                "suggested_service": "neptune",
                "reason": f"Method {method} exists in neptune but also in {service_count-1} other service(s) - verify correct usage",
                "python_method": method,
...
def main():
    ...
    with open(METADATA_MAPPING_FILE, 'r') as f:
        mapping_data = json.load(f)["neptune_metadata_mapping"]
    ...
    cross_service_count = len([s for s in cross_service if s.get("suggested_service") != "neptune"])
    ...
    if cross_service_count > 0:
        cross_service_confidence = sum(
            ...
            if s.get("suggested_service") != "neptune"
        ) / cross_service_count
    else:
