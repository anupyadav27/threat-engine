...
CREATE TABLE IF NOT EXISTS ciem_collection_watermark (
...
class EventWriter:
    ...
    def _build_db_url(self) -> str:
        ...
        port = os.getenv("CIEM_DB_PORT", os.getenv("DB_PORT", "5432"))
        name = os.getenv("CIEM_DB_NAME", "threat_engine_ciem")
        user = os.getenv("CIEM_DB_USER", os.getenv("DB_USER", "postgres"))
        pw = os.getenv("CIEM_DB_PASSWORD", os.getenv("INVENTORY_DB_PASSWORD", os.getenv("DB_PASSWORD", "")))
        return f"postgresql://{user}:{pw}@{host}:{port}/{name}"

    def ensure_table(self):
        """Create ciem_collection_watermark table if not exists."""
    ...
    def get_watermark(self, tenant_id: str, account_id: str, source_type: str) -> Optional[datetime]:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT watermark_at FROM ciem_collection_watermark
    ...
    def update_watermark(self, tenant_id: str, account_id: str, source_type: str, watermark_at: datetime) -> None:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO ciem_collection_watermark
