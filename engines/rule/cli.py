...
def cmd_list_services(args, config: Config):
    ...
    provider = args.provider or "aws"  # Default to AWS for backward compatibility
    print(f"\nProvider: {provider}")
    services = list_services(config, provider)
...
def cmd_list_fields(args, config: Config):
    """List fields for a service in a provider"""
    provider = args.provider or "aws"  # Default to AWS for backward compatibility
    
    if not config.validate_service(args.service, provider):
        print(f"Error: Service '{args.service}' not found or invalid for provider '{provider}'", file=sys.stderr)
        sys.exit(1)
    
    fields = list_fields(args.service, provider, config)
...
def cmd_generate(args, config: Config):
    """Generate YAML from JSON input or interactive mode with provider support"""
    provider = args.provider or "aws"  # Default to AWS for backward compatibility
    ...
    service_data = loader.load_service_data(args.service, provider)
    ...
    if args.input:
        ...
        selections = interactive_mode(args.service, provider, service_data, config)
    ...
    # Generate YAML with provider
    generator = YAMLGenerator(args.service, provider, service_data, config)
    
    # Check for existing rules with provider
    comparator = RuleComparator(args.service, provider, config)
    metadata_gen = MetadataGenerator(args.service, provider, config)
    ...
    # Generate YAML with all selections (both new and existing) using provider-aware paths
    output_path = None
    if args.output:
        output_path = Path(args.output)
    else:
        output_path = config.get_output_path(args.service, provider) / f"{args.service}.yaml"
    
    # Extract rule_id from first selection for logical operator
    logical_operator = "single"
    rule_id = selections[0].rule_id if selections else None
    
    yaml_str = generator.generate(selections, output_path, logical_operator=logical_operator, rule_id=rule_id)
...
def interactive_mode(service_name: str, provider: str, service_data: Dict, config: Config) -> List[FieldSelection]:
    """
    Interactive mode: Select Field + Operator + Expected Value together
    Now includes rule comparison and metadata collection with provider support
    """
    try:
        from .core.field_mapper import FieldMapper
        from .core.rule_comparator import RuleComparator
    ...
    # Initialize comparator and generator for rule checking with provider
    comparator = RuleComparator(service_name, provider, config)
    generator = YAMLGenerator(service_name, provider, service_data, config)
    
    print(f"\n{'='*60}")
    print(f"Interactive Rule Builder")
    ...
    while True:
        ...
        try:
            ...
            print(f"\nSTEP 5: Rule Details")
            default_rule_id = f"{provider}.{service_name}.resource.{field_name}_{operator.replace('_', '_')}"
            rule_id = input(f"Rule ID [{default_rule_id}]: ").strip() or default_rule_id
            
            # Validate rule_id starts with provider prefix
            if not rule_id.startswith(f"{provider}."):
                print(f"Warning: Rule ID should start with '{provider}.' prefix. Auto-correcting...")
                if rule_id.startswith(rule_id.split('.')[0] + '.'):
                    # Replace existing provider prefix
                    rule_id = f"{provider}." + '.'.join(rule_id.split('.')[1:])
                else:
                    # Add provider prefix
                    rule_id = f"{provider}.{rule_id}"
                print(f"Corrected Rule ID: {rule_id}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="YAML Rule Builder - Multi-CSP compliance rule generator with rule comparison",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # List available services (defaults to AWS)
  python -m yaml_rule_builder list-services
  python -m yaml_rule_builder list-services --provider aws

  # List fields for a service (defaults to AWS)
  python -m yaml_rule_builder list-fields --service accessanalyzer
  python -m yaml_rule_builder list-fields --provider aws --service accessanalyzer

  # Generate YAML interactively (with rule comparison, defaults to AWS)
  python -m yaml_rule_builder generate --service accessanalyzer
  python -m yaml_rule_builder generate --provider aws --service accessanalyzer

  # Generate YAML from JSON file
  python -m yaml_rule_builder generate --provider aws --service accessanalyzer --input rules.json --output accessanalyzer.yaml

       Supported Providers:
         - aws (default, fully implemented)
         - azure (fully implemented)
         - gcp (fully implemented)
         - oci (fully implemented)
         - alicloud (fully implemented)
         - ibm (fully implemented)
         - k8s (fully implemented)

Features:
  - Multi-CSP support with provider isolation
  - Automatically detects existing rules with same field + operator + value (two-phase matching)
  - YAML file merging (appends to existing files, doesn't overwrite)
  - Creates metadata files for new custom rules
  - Marks custom rules with 'custom: true' field
  - Provider-aware paths and discovery IDs
        """
    )
    
    subparsers = parser.add_subparsers(dest='command', help='Command to execute', required=True)
    
    list_services_parser = subparsers.add_parser('list-services', help='List available services for a provider')
    # Get available providers dynamically from Config
    try:
        temp_config = Config()
    ...
    list_services_parser.add_argument('--provider', 
                                     ...
                                     default='aws',
                                     help=f'Cloud Service Provider. Available: {", ".join(available_providers)} (default: aws)')
    
    list_fields_parser = subparsers.add_parser('list-fields', help='List fields for a service in a provider')
    list_fields_parser.add_argument('--provider',
                                   choices=available_providers,
                                   default='aws',
                                   help=f'Cloud Service Provider. Available: {", ".join(available_providers)} (default: aws)')
    list_fields_parser.add_argument('--service', required=True, help='Service name')
    
    generate_parser = subparsers.add_parser('generate', help='Generate YAML file with provider support')
