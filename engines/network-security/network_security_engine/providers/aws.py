"""
AWS network-security provider.

Contains the full 7-layer analysis previously embedded in run_scan.py
(lines 109-370).  Extracted verbatim; no logic was changed.
"""
...
class AWSNetworkProvider(BaseNetworkProvider):
    """Full 7-layer network security analysis for AWS (VPC/SG/NACL/LB/WAF)."""
    ...
    def analyze(
        ...
        if not discovery_data:
            ...
            return {"status": "skipped", "reason": "No discovery data"}

        # ── 4. Load inventory relationships (for SG attachments) ──────────
        sg_attachments: Dict[str, Any] = {}
        try:
        ...
        try:
            ...
            if ciem_net_findings:
                logger.info(
                    "CIEM: %d network findings from ciem_findings", len(ciem_net_findings)
                )
            ...
            snapshots.append(
                {
                    ...
                    "isolation_score": 100
                    - (50 if vpc.has_internet_gateway else 0)
        ...
        # ── 11. Compute scores / report metrics ───────────────────────────
        ...
        report_metrics: Dict[str, Any] = {
            ...
            "total_network_firewalls": sum(
                len(v.network_firewalls) for v in topology.vpcs.values()
            ),
            ...
            "cross_vpc_paths_count": sum(
                1
                for f in all_findings
                if f.effective_exposure == "cross_vpc" and f.status == "FAIL"
            ),
            ...
            "exposure_summary": {
                "internet_exposed": sum(
                    1 for f in all_findings if f.effective_exposure == "internet"
                ),
                "cross_vpc": sum(
                    1 for f in all_findings if f.effective_exposure == "cross_vpc"
                ),
                "vpc_internal": sum(
                    1 for f in all_findings if f.effective_exposure == "vpc_internal"
                ),
            },
        }

        logger.info(
            "AWS analysis complete: %d findings (%d critical, %d high) in %dms",
            len(all_findings),
