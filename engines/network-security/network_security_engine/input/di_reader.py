"""
Network DI Reader — reads from asset_inventory (threat_engine_di).

Drop-in replacement for NetworkDiscoveryReader. Returns the same
Dict[logical_name, List[row]] structure from load_all_network_resources()
so all network analyzers work without changes.
...
# Maps logical name → discovery_id (same as NetworkDiscoveryReader.NETWORK_DISCOVERY_MAP)
...
class NetworkDIReader:
    """Read network assets from asset_inventory in threat_engine_di."""
    ...
    def load_all_network_resources(
        ...
        """Load all network-relevant assets for a scan.

        Returns Dict keyed by logical name (same as NetworkDiscoveryReader).
        """
        ...
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                query = """
                    SELECT discovery_id, resource_uid, resource_uid AS resource_id,
                           region, service, raw_response, emitted_fields, config_hash
        ...
        logger.info(
            "NetworkDIReader: %d assets across %d categories for scan %s",
    ...
    def load_by_discovery_id(
        ...
        """Load assets for a single discovery_id."""
        conn = get_di_conn()
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                query = """
                    SELECT resource_uid, resource_uid AS resource_id,
                           region, service, raw_response, emitted_fields, config_hash
                    FROM asset_inventory
                    WHERE scan_run_id = %s
                      AND tenant_id = %s
