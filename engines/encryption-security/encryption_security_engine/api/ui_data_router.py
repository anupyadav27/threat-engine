...
def _resolve_latest_scan(cur, tenant_id: str) -> Optional[str]:
    ...
    Skips orphaned reports (no backing findings). Falls back to encryption_findings.
    """
    cur.execute(
        """
        SELECT r.scan_run_id FROM encryption_report r
        WHERE r.tenant_id = %s AND r.status = 'completed'
          AND EXISTS (
              SELECT 1 FROM encryption_findings f
