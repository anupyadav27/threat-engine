"""
Posture Scorer — Compute per-database and overall database security posture scores.

Scoring model:
  - Per-database, per-domain: PASS / (PASS + FAIL) ratio scaled to 0-100
...
def compute_posture_scores(
    ...
    db_inventory: Optional[List[Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    """Compute per-database and overall posture scores from check findings.

    Args:
        findings: List of check finding dicts.  Each must contain at least
                  ``rule_id``, ``resource_uid``, and ``status`` ('PASS'/'FAIL').
        db_inventory: Optional list of inventory entries. When provided, every
                      resource_uid in the inventory will appear in the output
                      even if it has zero findings.
    ...
    for f in findings:
        ...
        if service:
            db_services[resource_uid] = service

        db_findings_included += 1

        if status == "PASS":
            per_db[resource_uid][domain]["pass"] += 1
            global_counts[domain]["pass"] += 1
            total_pass += 1
        else:
            per_db[resource_uid][domain]["fail"] += 1
    ...
    if db_inventory:
        for inv in db_inventory:
            ...
            if uid and uid not in per_db:
                per_db[uid]  # creates defaultdict entry
            svc = inv.get("db_service")
            if uid and svc:
                db_services.setdefault(uid, svc)

    # ── Per-database scores ──────────────────────────────────────────────
    per_database_output: Dict[str, Dict[str, Any]] = {}
    databases_at_risk = 0

    for resource_uid, domain_map in per_db.items():
        d_scores = _compute_domain_scores(dict(domain_map))
        overall = _weighted_overall(d_scores)

        db_fail = sum(d["fail_count"] for d in d_scores.values())
        db_total = sum(d["total"] for d in d_scores.values())

        if db_fail > 0:
            databases_at_risk += 1

        per_database_output[resource_uid] = {
            "db_service": db_services.get(resource_uid, "unknown"),
            "overall_score": overall,
            "domain_scores": d_scores,
            "finding_count": db_total,
            "fail_count": db_fail,
        }
    ...
    return {
        ...
        "per_database": per_database_output,
        "summary": {
            "total_databases": len(per_db),
            "total_findings": db_findings_included,
            "total_pass": total_pass,
            "total_fail": total_fail,
            "databases_at_risk": databases_at_risk,
        },
    }
