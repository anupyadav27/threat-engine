...
def _resolve_latest_check_scan_id(conn, tenant_id: str) -> Optional[str]:
    """Resolve the latest check_scan_id for a tenant from check_report.

    Returns the most recent check_scan_id or None if no scans exist.
    """
    from psycopg2.extras import RealDictCursor
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        cur.execute("""
            SELECT check_scan_id
            FROM check_report
            WHERE tenant_id = %s
        ...
        """, (tenant_id,))
        row = cur.fetchone()
        return row["check_scan_id"] if row else None
...
async def get_findings_summary(
    ...
    try:
        # Auto-resolve to latest scan if no check_scan_id provided
        resolved_scan_id = check_scan_id
        if not resolved_scan_id:
            resolved_scan_id = _resolve_latest_check_scan_id(conn, tenant_id)
            logger.info(f"Auto-resolved latest check_scan_id: {resolved_scan_id}")

        # Build dynamic WHERE clause
        conditions = ["cf.tenant_id = %s"]
        params: list = [tenant_id]

        # Always filter by scan to avoid cross-scan duplicates
        if resolved_scan_id:
            conditions.append("cf.check_scan_id = %s")
            params.append(resolved_scan_id)
        ...
        if service:
            conditions.append("""
                COALESCE(
                    CASE WHEN cf.resource_arn LIKE 'arn:aws:%%:%%'
                         THEN split_part(cf.resource_arn, ':', 3) ELSE NULL END,
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            # 8. Account (hierarchy_id) breakdown
            cur.execute(f"""
                SELECT cf.hierarchy_id, cf.provider, COUNT(*) AS cnt,
                       SUM(CASE WHEN cf.status = 'FAIL' THEN 1 ELSE 0 END) AS fail_cnt
                {base_join} AND cf.hierarchy_id IS NOT NULL AND cf.hierarchy_id != ''
                GROUP BY cf.hierarchy_id, cf.provider
                ORDER BY cnt DESC
                LIMIT 50
            """, params)
        ...
        return {
            "total": total,
            "check_scan_id": resolved_scan_id,
            ...
            "by_account": [
                {"account": r["hierarchy_id"], "provider": r.get("provider") or "",
                 "total": int(r["cnt"]), "fail": int(r.get("fail_cnt") or 0)}
                for r in account_rows if r.get("hierarchy_id")
            ],
        }
    finally:
        conn.close()
...
async def list_findings(
    ...
    try:
        ...
        if posture_category:
            ...
            params.append(posture_category)
        if search:
            conditions.append("(rm.title ILIKE %s OR cf.rule_id ILIKE %s OR COALESCE(cf.resource_uid, cf.resource_arn) ILIKE %s)")
            like_val = f"%{search}%"
            params.extend([like_val, like_val, like_val])
        ...
        sort_map = {
            ...
            "region": "cf.region",
            "resource": "COALESCE(cf.resource_uid, cf.resource_arn)",
        }
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            cur.execute(f"""
                SELECT
                    ...
                    cf.provider,
                    cf.hierarchy_id,
                    COALESCE(cf.resource_uid, cf.resource_arn) AS resource_uid,
                    cf.resource_arn,
                    cf.resource_id,
                    cf.resource_type,
                    COALESCE(
...
async def get_findings_for_resource(
    ...
    try:
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            # Exact match
            cur.execute("""
                SELECT COALESCE(resource_uid, resource_arn) AS uid, check_scan_id
                FROM check_findings
                WHERE COALESCE(resource_uid, resource_arn) = %s
                  AND tenant_id = %s
            ...
            if not row and '/' not in resource_uid and ':' not in resource_uid:
                ...
                cur.execute("""
                    SELECT COALESCE(resource_uid, resource_arn) AS uid, check_scan_id
                    FROM check_findings
                    WHERE COALESCE(resource_uid, resource_arn) LIKE %s
                      AND tenant_id = %s
                    ORDER BY created_at DESC
                    LIMIT 1
        ...
        uid_match = "COALESCE(cf.resource_uid, cf.resource_arn) = %s"
        scan_filter = ""
        ...
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(f"""
                SELECT
                    ...
                    COALESCE(
                        CASE WHEN cf.resource_arn LIKE 'arn:aws:%%:%%'
                             THEN split_part(cf.resource_arn, ':', 3)
                             ELSE NULL END,
                        rm.resource_service, rm.service,
                        cf.resource_service, cf.service
                    ) AS service,
