# ECS Service Processing Prompt

## Task
Process ECS service metadata mapping and YAML generation following this sequence:

1. **Review all metadata from ECS**
   - Review all metadata files in `/Users/apple/Desktop/threat-engine/aws_compliance_python_engine/services/ecs/metadata/`
   - Understand security requirements and intent

2. **Create metadata_mapping.json**
   - Use boto3 database files from `/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/ecs/`
   - Reference:
...
3. **Review as AWS Python developer and security expert**
   - Verify methods exist in boto3 ECS client
...
4. **Generate YAML from metadata_mapping.json**
   - Run: `cd /Users/apple/Desktop/threat-engine/aws_compliance_python_engine && python3 generate_yaml_from_metadata_mapping.py ecs`
   - Verify YAML file is created in `services/ecs/rules/ecs.yaml`
...
5. **MANDATORY: Test with actual AWS resources**
   ...
   **Option B: Create test resources (if none exist)**
   - **REQUIRED**: Create a test script: `test_ecs_resources.py`
   ...
   **Step 5c: Run compliance scan**
   ...
   python3 -m aws_compliance_python_engine.engine.main_scanner --service ecs --region us-east-1
   ```
   ...
   **Step 5e: Analyze scan results**
   ...
   find $LATEST_SCAN -name "*.json" -path "*/ecs/*" | head -5 | xargs cat
   ```
...
6. **Detailed output analysis (MANDATORY)**
   ...
   **6c: Verify field paths in check evidence**
   ...
   find $LATEST_SCAN -name "*.json" -path "*/ecs/*" | head -3 | while read file; do
       echo "Checking: $file"
...
7. **Fix issues and retest (iterative process)**
   ...
   **7a: Fix identified issues**
   ...
   - If discovery errors: Fix `rules/ecs.yaml` directly or fix `metadata_mapping.json`
   ...
   **7b: Regenerate YAML (if metadata_mapping.json changed)**
   ```bash
   cd /Users/apple/Desktop/threat-engine/aws_compliance_python_engine
   python3 generate_yaml_from_metadata_mapping.py ecs
   ```
