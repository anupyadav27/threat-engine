#!/usr/bin/env python3
"""
Validate EIP YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "Addresses[].AllocationId" with items_for -> "item.AllocationId"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('Addresses.'):
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('eip_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'eip.yaml'
