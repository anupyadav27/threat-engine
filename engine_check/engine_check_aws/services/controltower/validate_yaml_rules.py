#!/usr/bin/env python3
"""
Validate CONTROLTOWER YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "enabledControls[].controlIdentifier" with items_for -> "item.controlIdentifier"
    - field_path: "enabledControlDetails.targetRegions" with items_for -> "item.targetRegions"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('enabledControls.'):
            clean_path = clean_path.replace('enabledControls.', '')
        elif clean_path.startswith('enabledControlDetails.'):
            clean_path = clean_path.replace('enabledControlDetails.', '')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('controltower_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'controltower.yaml'
