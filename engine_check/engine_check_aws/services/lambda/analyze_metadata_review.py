#!/usr/bin/env python3
"""
Lambda Metadata Review Analysis Script
Analyzes lambda service metadata for consolidation and cross-service placement opportunities.
"""
...
BOTO3_DEPS_FILE = Path("/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/lambda/boto3_dependencies_with_python_names_fully_enriched.json")
...
def check_method_ownership(method: str) -> Tuple[Optional[str], int]:
    ...
    try:
        ...
        # Check if method exists in lambda service
        lambda_ops = boto3_data.get("lambda", {}).get("operations", [])
        lambda_has_method = any(
        ...
        if lambda_has_method:
            if service_count > 0:
                return "lambda", service_count + 1  # Ambiguous
            return "lambda", 1  # Only in lambda
        elif service_count > 0:
            return first_other_service, service_count  # Not in lambda, in other service(s)
        else:
...
def extract_service_from_rule_id(rule_id: str) -> str:
    """Extract service name from rule_id (e.g., aws.lambda.function.x -> lambda)"""
    parts = rule_id.split(".")
...
def analyze_cross_service(mapping_data: List[Dict], metadata: Dict[str, Dict]) -> List[Dict]:
    ...
    for rule in mapping_data:
        ...
        if method_service and method_service != "lambda":
        ...
        elif method_service == "lambda" and service_count > 1:
            # Method exists in lambda but also in other services (ambiguous)
            suggestions.append({
                "rule_id": rule_id,
                "metadata_file": f"{rule_id}.yaml",
                "current_service": "lambda",
                "suggested_service": "lambda",
                "reason": f"Method {method} exists in lambda but also in {service_count-1} other service(s) - verify correct usage",
                "python_method": method,
...
def main():
    ...
    with open(METADATA_MAPPING_FILE, 'r') as f:
        mapping_data = json.load(f)["lambda_metadata_mapping"]
    ...
    cross_service_count = len([s for s in cross_service if s.get("suggested_service") != "lambda"])
    ...
    if cross_service_count > 0:
        cross_service_confidence = sum(
            ...
            if s.get("suggested_service") != "lambda"
        ) / cross_service_count
    else:
