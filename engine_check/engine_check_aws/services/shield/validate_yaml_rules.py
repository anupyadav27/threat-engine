#!/usr/bin/env python3
"""
Validate SHIELD YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "Protections[].ResourceArn" with items_for -> "item.ResourceArn"
    - field_path: "Subscription.AutoRenew" -> "item.Subscription.AutoRenew"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('Protections.'):
            clean_path = clean_path.replace('Protections.', '')
        elif clean_path.startswith('LogBucketList.'):
            clean_path = clean_path.replace('LogBucketList.', '')
        return f"item.{clean_path}" if clean_path else "item"
    else:
        # If not items_for, check if emit extracts from a specific path
        # For describe_subscription, emit shows Subscription fields directly
        # but YAML may use item.Subscription.*, so keep the full path
        if clean_path.startswith('Subscription.'):
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('shield_metadata_mapping', []):
    ...
    for i, expected_field in enumerate(expected_fields):
        ...
        # Handle empty field path (for checking item itself)
        if expected_path == '':
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'shield.yaml'
