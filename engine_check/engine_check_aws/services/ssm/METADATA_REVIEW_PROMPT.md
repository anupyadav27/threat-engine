# ssm Metadata Review and Consolidation Prompt

You are an expert AWS compliance engineer reviewing metadata mappings and metadata files for the **ssm** service. Your task is to analyze and provide actionable suggestions for consolidations and cross-service placements.

## Service Context

**Service Name**: `ssm`  
**Service Path**: `/Users/apple/Desktop/threat-engine/aws_compliance_python_engine/services/ssm/`
...
**Path**: `/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/ssm/boto3_dependencies_with_python_names_fully_enriched.json`

Contains actual AWS API method specifications to validate method ownership.

## Your Task

Analyze the ssm service and provide:
...
{{
  ...
  "consolidation_suggestions": {{
    "duplicates": [
      {{
        "keep": {{
          "rule_id": "aws.ssm.resource.check_complete",
          "metadata_file": "aws.ssm.resource.check_complete.yaml",
        ...
        "remove": [
          {{
            "rule_id": "aws.ssm.resource.check_basic",
            "metadata_file": "aws.ssm.resource.check_basic.yaml",
            "replaced_by": "aws.ssm.resource.check_complete",
  ...
  "cross_service_suggestions": [
    {{
      "rule_id": "aws.ssm.resource.check_name",
      "metadata_file": "aws.ssm.resource.check_name.yaml",
      "current_service": "ssm",
...
2. **Service Prefix Matching**
   - Extract service from rule_id: `aws.ssm.{{resource}}.{{check}}`
...
1. **Load all files** for ssm service
...
**Begin Review**: Analyze the ssm service files and generate comprehensive review JSON.
