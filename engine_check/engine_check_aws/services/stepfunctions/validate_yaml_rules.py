#!/usr/bin/env python3
"""
Validate STEPFUNCTIONS YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "stateMachines[].name" with items_for -> "item.name"
    - field_path: "loggingConfiguration.level" -> "item.loggingConfiguration.level"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('stateMachines.'):
            clean_path = clean_path.replace('stateMachines.', '')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('stepfunctions_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'stepfunctions.yaml'
