#!/usr/bin/env python3
"""
Validate ELASTICBEANSTALK YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "OptionSettings[].Namespace" with items_for -> "item.OptionSettings[].Namespace"
    - field_path: "ApplicationVersions[].SourceBundle.S3Bucket" -> "item.SourceBundle.S3Bucket"
    """
    # Check if emit uses items_for (array iteration)
    uses_items_for = 'items_for' in emit_structure
    
    # Remove array notation from the beginning if it matches the response_path
    clean_path = field_path
    
    # Handle OptionSettings array - it's nested within the item
    if 'OptionSettings[]' in clean_path:
        # OptionSettings is already part of item, so keep it as is
        return f"item.{clean_path}"
    
    # Handle ApplicationVersions array
    if clean_path.startswith('ApplicationVersions[].'):
        clean_path = clean_path.replace('ApplicationVersions[].', '')
        return f"item.{clean_path}"
    
    # Handle Applications array
    if clean_path.startswith('Applications[].'):
        clean_path = clean_path.replace('Applications[].', '')
        return f"item.{clean_path}"
    
    # Handle ConfigurationSettings array
    if clean_path.startswith('ConfigurationSettings[].'):
        clean_path = clean_path.replace('ConfigurationSettings[].', '')
        return f"item.{clean_path}"
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('elasticbeanstalk_metadata_mapping', []):
    ...
    if 'any' in conditions:
        # YAML has any wrapper - this is for checking if ANY option setting matches
        inner_conditions = conditions['any']
        if isinstance(inner_conditions, list) and len(inner_conditions) > 0:
            # Check the inner structure
            first_inner = inner_conditions[0]
            if 'all' in first_inner:
                # YAML structure: any -> [all -> conditions]
                # This is checking if ANY option setting matches ALL conditions
                # If metadata says logical_operator: "any", this is correct
                # If metadata says logical_operator: "all", it means the inner all
                if expected_op == 'any':
                    # Correct - outer any matches metadata
                    pass
                elif expected_op == 'all':
                    # Metadata says all, but YAML has any wrapper
                    # This is actually correct - the any is for iterating OptionSettings
                    # The inner all is what metadata refers to
                    pass
                elif expected_op is None:
                    issues.append('Expected single condition but found "any" wrapper with "all" inside')
            else:
                # Single condition in any
                if expected_op == 'all':
    ...
    # For "any" wrapper with "all" inside, use the first "all" group for validation
    # (since both groups check the same structure, just different values)
    if 'any' in conditions:
        inner_list = conditions['any']
        if isinstance(inner_list, list) and len(inner_list) > 0:
            if 'all' in inner_list[0]:
                # Use first all group for validation (both groups have same structure)
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'elasticbeanstalk.yaml'
