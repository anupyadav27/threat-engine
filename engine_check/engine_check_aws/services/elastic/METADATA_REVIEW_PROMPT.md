# elastic Metadata Review and Consolidation Prompt

You are an expert AWS compliance engineer reviewing metadata mappings and metadata files for the **elastic** service. Your task is to analyze and provide actionable suggestions for consolidations and cross-service placements.

## Service Context

**Service Name**: `elastic`  
**Service Path**: `/Users/apple/Desktop/threat-engine/aws_compliance_python_engine/services/elastic/`
...
**Path**: `/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/elastic/boto3_dependencies_with_python_names_fully_enriched.json`

Contains actual AWS API method specifications to validate method ownership.

## Your Task

Analyze the elastic service and provide:
...
{{
  ...
  "consolidation_suggestions": {{
    "duplicates": [
      {{
        "keep": {{
          "rule_id": "aws.elastic.resource.check_complete",
          "metadata_file": "aws.elastic.resource.check_complete.yaml",
        ...
        "remove": [
          {{
            "rule_id": "aws.elastic.resource.check_basic",
            "metadata_file": "aws.elastic.resource.check_basic.yaml",
            "replaced_by": "aws.elastic.resource.check_complete",
  ...
  "cross_service_suggestions": [
    {{
      "rule_id": "aws.elastic.resource.check_name",
      "metadata_file": "aws.elastic.resource.check_name.yaml",
      "current_service": "elastic",
...
2. **Service Prefix Matching**
   - Extract service from rule_id: `aws.elastic.{{resource}}.{{check}}`
...
1. **Load all files** for elastic service
...
**Begin Review**: Analyze the elastic service files and generate comprehensive review JSON.
