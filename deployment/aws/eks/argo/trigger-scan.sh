...
#   Discovery only:    bash trigger-scan.sh --discovery <scan-run-id> <tenant-id> <account-id> [provider] [services]
...
#
# Examples (AWS):
#   bash trigger-scan.sh $(python3 -c "import uuid; print(uuid.uuid4())") test-tenant-002 588989875114
#   bash trigger-scan.sh $(python3 -c "import uuid; print(uuid.uuid4())") test-tenant-002 588989875114 aws threat-engine/account/588989875114
#
# Examples (Azure):
...
case "${1:-}" in
  --discovery)
    # Discovery-only: creates orchestration record then runs just discovery.
    # Use this to validate discovery output per CSP before full pipeline.
    SCAN_ID="${2:?Usage: trigger-scan.sh --discovery <scan-run-id> <tenant-id> <account-id> [provider] [services]}"
    TENANT_ID="${3:?Missing tenant-id}"
    ACCOUNT_ID="${4:?Missing account-id}"
    PROVIDER="${5:-aws}"
    INCLUDE_SERVICES="${6:-}"

    # CSP credentials are stored under provider-specific paths in Secrets Manager.
    if [ "$PROVIDER" = "azure" ]; then
      CREDENTIAL_TYPE="service_principal"
      CREDENTIAL_REF="threat-engine/azure/${ACCOUNT_ID}"
    elif [ "$PROVIDER" = "gcp" ]; then
      CREDENTIAL_TYPE="service_account"
      CREDENTIAL_REF="threat-engine/gcp/${ACCOUNT_ID}"
    elif [ "$PROVIDER" = "alicloud" ]; then
      ...
      # K8s uses in-cluster service account — no Secrets Manager needed
      CREDENTIAL_TYPE="in_cluster"
      CREDENTIAL_REF="in_cluster"
    else
      CREDENTIAL_TYPE="access_key"
      CREDENTIAL_REF="threat-engine/account/${ACCOUNT_ID}"
    fi

    echo "=== Triggering discovery-only for provider=$PROVIDER ==="
    ...
    argo submit -n $NS \
      ...
      --entrypoint discovery-only \
      --generate-name "cspm-disc-${PROVIDER}-" \
      ...
      --labels "scan-run-id=$SCAN_ID,mode=discovery-only"

    echo ""
    echo "Watch: $0 --watch cspm-disc-${PROVIDER}-<suffix>"
    ;;
  ...
  ""|--help)
    ...
    echo "  Discovery only: $0 --discovery <scan-run-id> <tenant-id> <account-id> [provider] [services]"
    echo "  Single engine:  $0 --engine <name> <scan-run-id>"
    echo "  Watch:          $0 --watch <workflow-name>"
    echo "  List recent:    $0 --list"
    echo ""
    echo "  provider: aws (default) | azure | gcp | alicloud | oci | k8s | ibm"
    echo "  --discovery creates its own orchestration record (fresh scan)"
    echo "  --engine    re-runs against an existing scan_run_id"
    echo ""
    echo "Engines: discovery, check, inventory, threat, compliance, iam, datasec,"
    echo "         encryption, database-security, container-security, network-security,"
    echo "         ciem, ai-security, risk, secops"
    ;;
