#!/usr/bin/env python3
"""
Validate WELLARCHITECTED YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "WorkloadSummaries[].WorkloadId" with items_for -> "item.WorkloadId"
    - field_path: "Workload.RiskCounts.HIGH" -> "item.RiskCounts.HIGH"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('WorkloadSummaries.'):
            clean_path = clean_path.replace('WorkloadSummaries.', '')
        return f"item.{clean_path}" if clean_path else "item"
    else:
        # If not items_for, check if emit extracts from a specific path
        if clean_path.startswith('Workload.'):
            clean_path = clean_path.replace('Workload.', '')
        return f"item.{clean_path}" if clean_path else "item"
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('wellarchitected_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'wellarchitected.yaml'
