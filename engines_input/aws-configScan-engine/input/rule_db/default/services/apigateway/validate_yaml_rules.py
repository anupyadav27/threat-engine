#!/usr/bin/env python3
"""
Validate APIGATEWAY YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "items[].enabled" with items_for -> "item.enabled"
    - field_path: "enabled" with items_for -> "item.enabled"
    - field_path: "accessLogSettings.format" with item (not items_for) -> "item.accessLogSettings.format"
    """
    ...
    if uses_items_for:
        # If items_for, each item is already 'item', so remove 'items.' prefix if present
        if clean_path.startswith('items.'):
            clean_path = clean_path.replace('items.', '')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('apigateway_metadata_mapping', []):
    ...
    if discovery_id and expected_method:
        # Extract method from discovery_id (e.g., aws.apigateway.get_api_keys -> get_api_keys)
        actual_method = discovery_id.split('.')[-1] if '.' in discovery_id else discovery_id
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'apigateway.yaml'
