...
def normalize_db_check_results_to_findings(
    ...
    """
    Normalize check results from database to misconfig findings.
    
    Args:
        check_results: List of check result dictionaries from database
                      Should be enriched with metadata (severity, title, etc.) via JOIN with rule_metadata
        cloud: Cloud provider
        include_metadata: If True, expects metadata fields in check_results (from JOIN)
    
    Returns:
        List of normalized MisconfigFinding objects
    
    Note:
        For best performance, query check_results with JOIN to rule_metadata:
        SELECT cr.*, rm.severity, rm.title, rm.description, rm.remediation
    ...
    for check in check_results:
        ...
        if resource_arn:
            ...
            # Try to extract from resource_uid
            parts = resource_uid.split(":")
            if len(parts) >= 3:
                region = parts[2] if parts[2] else "global"
        
        # Extract service from resource_type (e.g., "aws_s3_bucket" -> "s3")
        service = "unknown"
        if resource_type:
            # Remove provider prefix (aws_, azure_, gcp_, etc.)
            service = resource_type.replace(f"{cloud.value}_", "").split("_")[0]
        ...
        if include_metadata and check.get("severity"):
            # Metadata from database JOIN
            severity_str = check.get("severity", "medium").lower()
        else:
            # Fallback: try finding_data or use status-based mapping
            severity_str = finding_data.get("severity", "medium").lower()
        
        try:
            severity = Severity(severity_str)
        except ValueError:
            # Map status to severity if severity not found
            if status == "FAIL":
