...
class Rule:
    ...
    rule_id: str
    service: str
    provider: str  # REQUIRED: Provider name (e.g., 'aws', 'azure', 'gcp')
    title: str
    description: str
    remediation: str
    ...
    def __post_init__(self):
        """Validate rule including provider"""
        if not self.rule_id:
        ...
        # Validate rule_id matches provider prefix
        expected_prefix = f"{self.provider}."
        if not self.rule_id.startswith(expected_prefix):
            raise ValueError(
                f"rule_id '{self.rule_id}' must start with provider prefix '{expected_prefix}'"
            )
    ...
    def from_dict(cls, data: dict) -> "Rule":
        ...
        return cls(
            ...
            provider=data.get("provider") or data["rule_id"].split(".")[0],  # Extract from rule_id if not provided
            title=data["title"],
