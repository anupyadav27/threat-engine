...
class DependencyResolver:
    ...
    def __init__(self, service_name: str, provider: str, service_data: Dict, config=None):
        ...
        Args:
            ...
            config: Optional Config instance for provider adapter
        """
        self.service_name = service_name
        self.provider = provider
        self.config = config
        
        # Get provider adapter if config provided
        if config:
            try:
        ...
        # Use generic key names (works for all providers)
        deps_key = "boto3_deps" if "boto3_deps" in service_data else list(service_data.keys())[0]
        self.boto3_deps = service_data.get(deps_key) or service_data.get(f"{provider}_deps") or service_data
        self.dependency_index = service_data.get("dependency_index", {})
        
        # Get service data from deps
        service_key = service_name
        if isinstance(self.boto3_deps, dict):
        ...
        self.service_data = self.boto3_deps.get(service_key, {}) if isinstance(self.boto3_deps, dict) else {}
    ...
    def build_discovery_chain(self, operation_chain: List[str]) -> Optional[DiscoveryChain]:
        ...
        # Use provider adapter to format discovery_id
        if self.provider_adapter:
            discovery_id = self.provider_adapter.format_discovery_id(
                self.service_name,
                root_data['python_method']
            )
        else:
            # Fallback for backward compatibility (AWS format)
