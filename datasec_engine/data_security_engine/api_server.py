...
class ScanRequest(BaseModel):
    ...
    scan_id: str = Field(..., description="ConfigScan scan ID")
...
async def generate_report(request: ScanRequest):
    ...
    """
    try:
        logger.info(f"Generating data security report for scan: {request.scan_id}")
...
async def get_access_governance(
    ...
    try:
        ...
        logger.error(f"Error getting access governance: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_classification(
    ...
    """Get data classification results (PII/PCI/PHI detection)."""
    try:
...
async def get_lineage(
    ...
    """Get data lineage (flow tracking across services)."""
    try:
...
async def get_residency(
    ...
    """Get data residency compliance (geographic location checks)."""
    try:
...
async def get_compliance(
    ...
    """Get data compliance status (GDPR, PCI, HIPAA compliance checks)."""
    try:
        # Get data-related findings
        findings = configscan_reader.filter_data_related_findings(csp, scan_id)
...
async def get_findings(
    ...
    """Get all data security findings with optional filtering."""
    try:
        # Get data-related findings
        findings = configscan_reader.filter_data_related_findings(csp, scan_id)
...
async def get_account_data_security(
    ...
    """Get comprehensive data security status for a specific account."""
    try:
...
async def get_service_data_security(
    ...
    """Get comprehensive data security status for a specific service."""
    try:
        # Get findings for service
        findings = configscan_reader.filter_data_related_findings(csp, scan_id, services=[service.lower()])
