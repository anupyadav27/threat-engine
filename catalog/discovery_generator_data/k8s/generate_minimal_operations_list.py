...
def find_minimal_operations(
    ...
    # First, get all entities that are actually needed from direct_vars
    entities_from_fields = set()
    field_to_entities = {}
    for field_name, field_data in all_fields.items():
        ...
        if field_data.get("dependency_index_entity"):
            ...
            entities.add(entity)
            entities_from_fields.add(entity)
        # Add produces
        for prod in field_data.get("produces", []):
            entities.add(prod)
            entities_from_fields.add(prod)
        field_to_entities[field_name] = entities
    
    # Get all producible entities from dependency_index
    producible_entities = set()
    roots = dependency_index.get("roots", [])
    for root in roots:
        producible_entities.update(root.get("produces", []))
    
    entity_paths = dependency_index.get("entity_paths", {})
    producible_entities.update(entity_paths.keys())
    for entity, paths in entity_paths.items():
        for path in paths:
            produces = path.get("produces", {})
            for op, entities in produces.items():
                producible_entities.update(entities)
    
    # Only count entities that are both needed (from fields) AND producible
    all_entities_needed = entities_from_fields & producible_entities
    
    # If no entities from fields, fall back to all producible entities
    if not all_entities_needed and producible_entities:
        all_entities_needed = producible_entities.copy()
...
def is_read_operation_by_name(op_name: str) -> bool:
    ...
    Based on k8s patterns:
    - list operations: "list"
    - get operations: "get", "read"
    """
    if not op_name:
        return False
    
    op_lower = op_name.lower()
    read_ops = ['list', 'get', 'read']
    
    return op_lower in read_ops

def build_fallback_dependency_index_from_k8s(service_name: str, service_dir: Path) -> Optional[Dict]:
    """Build a minimal dependency_index from k8s_dependencies when dependency_index is empty.
    Includes operations that:
    1. Have item_fields (preferred), OR
    2. Have names that suggest read operations (list, get, read)
    """
    ...
    for op_data in all_operations:
        ...
        # "list" operations are typically independent
        # "get" operations might be dependent (need an ID)
        is_independent = op_name.lower() == 'list'
        
        # k8s operations are simple: "list", "get", "read"
        # Entity name format: k8s.service.operation
        # But we need to use the full operation name from dependency_index format
        # k8s operations in dependency_index are like "k8s.pod.list"
        full_op_name = f"k8s.{service_name}.{op_name}"
        
        # Create entity name: k8s.service (the resource itself)
        entity_name = f"k8s.{service_name}"
        ...
        entity_paths[entity_name].append({
            "operations": [full_op_name],
            "produces": {full_op_name: [entity_name]},
            "consumes": {},
            "external_inputs": []
        })
...
def generate_operations_report(service_name: str, service_dir: Path) -> Optional[Dict]:
    ...
    # If dependency_index is empty, try to build from k8s_dependencies
    if dependency_index is None:
        dependency_index = build_fallback_dependency_index_from_k8s(service_name, service_dir)
        if dependency_index is None:
...
def generate_all_services(base_dir: Path):
    ...
    print("="*80)
    print("GENERATING MINIMAL OPERATIONS LIST FOR ALL K8S SERVICES")
    print("="*80)
    
    # Get all service directories - include those with dependency_index OR k8s_dependencies
