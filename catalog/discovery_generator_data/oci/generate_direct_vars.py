#!/usr/bin/env python3
"""
Generate direct_vars.json for OCI services from operation_registry.json.
...
def generate_discovery_id(service_name: str, operation_id: str) -> str:
    ...
    # Operation ID is like "get_governance_instance" - already snake_case
    # Or could be "GetGovernanceInstance" - convert to snake_case
    op_snake = to_snake_case(operation_id) if any(c.isupper() for c in operation_id) else operation_id
    return f"oci.{service_name}.{op_snake}"

def extract_field_name_from_entity(entity: str, service_name: str) -> str:
    """Extract field name from entity path"""
    # Entity format: "oci.service.entity_name" (e.g., "oci.access_governance_cp.governance_instance.name")
    # Extract field name (last part after service)
    parts = entity.split('.')
    
    if len(parts) >= 3 and parts[0] == "oci" and parts[1] == service_name:
        # Remove "oci.{service}" prefix, keep the rest as field path
        field_name = '.'.join(parts[2:])
        return field_name
    elif len(parts) >= 2 and parts[0] == "oci":
        # Handle common entities like "oci.ocid", "oci.compartment_id"
        if parts[1] in ["ocid", "compartment_id"]:
            return parts[1]
        # Otherwise, take everything after "oci."
        return '.'.join(parts[1:])
...
def extract_field_type_from_path(path: str) -> str:
    ...
    if 'time' in path_lower or 'date' in path_lower or 'created' in path_lower or 'updated' in path_lower:
        return "string"  # date-time format
    elif 'id' in path_lower or 'ocid' in path_lower or 'compartment_id' in path_lower:
        return "string"
    elif 'status' in path_lower or 'state' in path_lower or 'lifecycle_state' in path_lower:
        return "string"
    elif 'tags' in path_lower or 'freeform_tags' in path_lower or 'defined_tags' in path_lower:
        return "object"  # OCI uses object for tags
    elif 'token' in path_lower or 'next_token' in path_lower:
        return "string"
    else:
        return "string"
...
def generate_direct_vars_from_registry(service_name: str, service_dir: Path) -> Optional[Dict]:
    ...
    for entity, field_name in entity_to_field_name.items():
        ...
        is_get = any("get" in op.lower() for op in operations_list)
    ...
    for field_name, field_data in fields.items():
        ...
        is_get_field = any("get" in op.lower() for op in operations)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate direct_vars.json for OCI services")
    ...
    print("="*80)
    print("GENERATING DIRECT_VARS.JSON FOR OCI SERVICES")
    print("="*80)
    print()
