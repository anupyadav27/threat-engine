#!/usr/bin/env python3
"""
Generate dependency_index.json for OCI services from operation_registry.json.

OCI dependency_index structure (similar to IBM/Alicloud):
{
  "service": "servicename",
  "read_only": true,
  "roots": [
...
def build_roots_from_registry(operation_registry: Dict, service_name: str) -> List[Dict[str, Any]]:
    ...
    for op_id, op_data in read_ops.items():
        ...
        # In OCI, consumes with "source": "external" are external inputs, not dependencies
        # Also check for "internal" source - those are real dependencies
        required_consumes = [c for c in consumes if c.get("required", True) and c.get("source") == "internal"]
...
def build_entity_paths_from_registry(operation_registry: Dict, service_name: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for op_id, op_data in read_ops.items():
        ...
        consumes_entities = [c.get("entity", "") for c in consumes if c.get("required", True) and c.get("source") == "internal"]
    ...
    # Convert to list format matching OCI structure
    result = {}
...
def generate_dependency_index(service_name: str, service_dir: Path) -> Optional[Dict]:
    """Generate dependency_index.json for an OCI service"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate dependency_index.json for OCI services")
    ...
    print("="*80)
    print("GENERATING DEPENDENCY_INDEX.JSON FOR OCI SERVICES")
    print("="*80)
    print()
