#!/usr/bin/env python3
"""
Generate dependency_index.json for Alicloud services from operation_registry.json.

Alicloud dependency_index structure:
{
  "service": "servicename",
  "read_only": true,
  "roots": [
    ...
    "entity1": [
      {
        ...
        "produces": {"OperationName": ["entity1"]},
        "consumes": {"OperationName": []},
        "external_inputs": []
      }
    ]
  }
...
def build_roots_from_registry(operation_registry: Dict, service_name: str) -> List[Dict[str, Any]]:
    ...
    for op_id, op_data in read_ops.items():
        ...
        # In Alicloud, consumes with "source": "external" are external inputs, not dependencies
        required_consumes = [c for c in consumes if c.get("required", True) and c.get("source") != "external"]
...
def build_entity_paths_from_registry(operation_registry: Dict, service_name: str) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for op_id, op_data in read_ops.items():
        ...
        consumes_entities = [c.get("entity", "") for c in consumes if c.get("required", True) and c.get("source") != "external"]
        consumes_entities = [e for e in consumes_entities if e]
        
        # Get external inputs separately
        external_inputs = [c.get("entity", "") for c in consumes if c.get("source") == "external"]
        ...
        for entity in produces_entities:
            ...
            if external_inputs and entity not in entity_paths[entity]["external_inputs"]:
                entity_paths[entity]["external_inputs"].extend(external_inputs)
    
    # Convert to list format matching Alicloud structure
    result = {}
...
def generate_dependency_index(service_name: str, service_dir: Path) -> Optional[Dict]:
    """Generate dependency_index.json for an Alicloud service"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate dependency_index.json for Alicloud services")
    ...
    print("="*80)
    print("GENERATING DEPENDENCY_INDEX.JSON FOR ALICLOUD SERVICES")
    print("="*80)
    print()
