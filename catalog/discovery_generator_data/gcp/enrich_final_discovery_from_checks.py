#!/usr/bin/env python3
"""
enrich_final_discovery_from_checks.py
======================================
For each GCP final_discovery_v1.yaml, scan the check rules that reference
each discovery op and add any missing top-level fields to emit.item.

This ensures that every `var: item.FIELD.subfield` in a check rule has
its parent `FIELD` present in the corresponding emit.item block so the
check engine can traverse to the subfield.

Does NOT touch aggregatedList ops (they have empty item blocks by design).
"""
...
def build_required_fields() -> Dict[str, Set[str]]:
    """Returns {discovery_op → set(top_level_fields)} from all GCP check rules."""
    ...
    for svc_dir in CHECK_DIR.iterdir():
        ...
        for rule in data.get('checks', []):
            ...
            for var in extract_vars(rule.get('conditions')):
                ...
                if top and top not in ('', 'metadata_items'):
...
def patch_file(yaml_path: Path, required: Dict[str, Set[str]]) -> int:
    ...
    # We'll rebuild the file line-by-line, inserting missing fields
    # after the last existing `item.` key in each emit.item block.
    # Approach: parse the YAML, patch, re-render the affected blocks in the raw text.
    ...
    for disc in data.get('discovery', []):
        ...
        # Skip aggregatedList ops
        if did.lower().endswith('aggregatedlist'):
            continue
        if item is None:
            continue  # no emit.item block
    ...
    if patched and not DRY_RUN:
        # Re-render the file using our custom renderer
...
def _rewrite_file(yaml_path: Path, data: dict):
    """
    Rewrite the final_discovery_v1.yaml preserving structure and comments.
    We use a targeted approach: keep the header comments + rewrite the YAML body.
    """
    # Read original to preserve header comment block
...
def _render_final_discovery(data: dict) -> List[str]:
    ...
    lines.append(f"provider: {data.get('provider', 'gcp')}")
    ...
    for disc in data.get('discovery', []):
        ...
        if items_for:
            ...
            pass  # aggregatedList — no item block
