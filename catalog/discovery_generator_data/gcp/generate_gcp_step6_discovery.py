#!/usr/bin/env python3
"""
generate_gcp_step6_discovery.py
================================
Generate {svc}.discovery.yaml for every GCP service that has check rules.

Output: catalog/rule/gcp_rule_check/{svc}/{svc}.discovery.yaml

Usage:
    python generate_gcp_step6_discovery.py             # dry-run
    python generate_gcp_step6_discovery.py --apply     # write all
    python generate_gcp_step6_discovery.py --svc compute --apply
"""
...
OPS_CSV   = GEN_DIR / 'gcp_master_read_ops.csv'
...
print('Loading gcp_master_read_ops.csv ...')
...
def action_from_python_call(python_call: str) -> str:
    """
    svc.instances().list(**params).execute() → instances.list
    svc.projects().locations().instances().list(**params).execute() → projects.locations.instances.list
    """
    if not python_call:
        return ''
    # Strip svc. prefix and (**params).execute() or ().execute() suffix
    s = python_call.strip()
    s = re.sub(r'^svc\.', '', s)
    s = re.sub(r'\(\*\*params\)\.execute\(\)\s*$', '', s)
    s = re.sub(r'\(\)\.execute\(\)\s*$', '', s)
    # Now s looks like: instances().list  or  projects().locations().instances().list
    # Remove all () and join with dots
    parts = re.split(r'\(\)\.?', s)
...
def build_items_for(op: str, row: dict) -> str:
    """Resolve items_for from pre-loaded data or defaults."""
    if op in op_items_for:
        return op_items_for[op]
    op_kind = row.get('op_kind', '')
    if op_kind == 'read_get':
        return '{{ response }}'
    # Default for GCP list ops
...
def build_emit_item(produced_fields: str, op_kind: str) -> dict:
    ...
    for field in fields:
        ...
        top = field.split('.')[0] if '.' in field else field
