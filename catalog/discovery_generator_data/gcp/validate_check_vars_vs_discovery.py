#!/usr/bin/env python3
"""
validate_check_vars_vs_discovery.py
====================================
For every GCP check rule, verify that the `var:` field it checks is
actually emitted by the corresponding final_discovery_v1.yaml.

Logic:
  rule.var = "item.autoclass.enabled"
    → top-level field = "autoclass"
    → PASS if "autoclass" in emit.item keys for that op
    → also PASS if op is aggregatedList (fields come from raw response)
...
SERVICE_ALIASES: Dict[str, str] = {
    'audit': 'logging', 'bigtable': 'bigtableadmin', 'billing': 'billingbudgets',
    'endpoints': 'servicemanagement', 'filestore': 'file', 'gke': 'container',
    'kms': 'cloudkms', 'resourcemanager': 'cloudresourcemanager',
    'security_command_center': 'securitycenter', 'sql': 'sqladmin',
}
# ──────────────────────────────────────────────────────────────────────────────
...
def load_all_emit_fields() -> Dict[str, Set[str]]:
    ...
    Aggregated-list ops flagged with '__aggregated__'.
    """
    ...
    for yaml_path in GEN_DIR.glob('*/final_discovery_v1.yaml'):
        ...
        for disc in data.get('discovery', []):
            ...
            emit = disc.get('emit', {})
            item = emit.get('item', {})
            if did.lower().endswith('aggregatedlist') or \
               (not emit.get('items_for') and not item):
                result[did] = {'__aggregated__'}
            else:
                keys: Set[str] = set()
...
def check_var(var: str, emit_keys: Set[str]) -> str:
    """
    Returns 'PASS', 'AGGR' (aggregated — can't statically verify), or 'MISS'.
    """
    if '__aggregated__' in emit_keys:
        return 'AGGR'
    ...
    # Special: metadata_items.key → always flat dict on item
    if top == 'metadata_items':
        return 'PASS'   # we explicitly added metadata_items to emit
...
# Counters
total_vars = pass_count = miss_count = aggr_count = skip_count = 0

miss_details: List[Tuple[str, str, str, str]] = []  # (svc, op, var, reason)
aggr_details: List[Tuple[str, str, str]]       = []  # (svc, op, var)
...
for svc in check_services:
    ...
    svc_pass = svc_miss = svc_aggr = svc_skip = 0
    ...
    for op, vars_ in check_vars.items():
        ...
        for v in vars_:
            ...
            if result == 'PASS':
                ...
                svc_aggr += 1
                aggr_count += 1
                aggr_details.append((svc, op, v))
            else:
                svc_miss += 1
                miss_count += 1
                miss_details.append((svc, op, v, f'top={v.removeprefix("item.").split(".")[0]} not in emit'))

    svc_summary.append((svc, gen_svc, svc_pass, svc_miss, svc_aggr, svc_skip))


# ── Print report ──────────────────────────────────────────────────────────────

print('═' * 75)
print('GCP  Check-rule var → discovery emit field validation')
print('═' * 75)
print(f'{"Service":<28} {"Gen-svc":<22} {"PASS":>5} {"MISS":>5} {"AGGR":>5} {"SKIP":>5}')
print('-' * 75)
...
print('═' * 75)
print(f'{"TOTAL":<28} {"":<22} {pass_count:>5} {miss_count:>5} {aggr_count:>5} {skip_count:>5}')
print()
print('Legend: PASS=field in emit  MISS=field not found  AGGR=aggregatedList(runtime-only)  SKIP=no discovery yaml')
...
if aggr_details and '--show-aggr' in sys.argv:
    print()
    print('─' * 75)
    print('AGGR — aggregatedList ops (fields verified at runtime, not statically):')
    print('─' * 75)
    for svc, op, var in aggr_details[:30]:
        print(f'  [{svc}] {op}  var={var}')
    if len(aggr_details) > 30:
        print(f'  ... and {len(aggr_details)-30} more')
