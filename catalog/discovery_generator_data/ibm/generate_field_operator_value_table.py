#!/usr/bin/env python3
"""
Generate field_operator_value_table.csv for IBM Cloud services.
Adapted from alicloud version for IBM structure.
"""
...
def load_service_data(service_name: str, base_dir: Path) -> Dict[str, Any]:
    ...
    # Load ibm_dependencies_with_python_names_fully_enriched.json
    ibm_deps_path = service_dir / 'ibm_dependencies_with_python_names_fully_enriched.json'
    if ibm_deps_path.exists():
        with open(ibm_deps_path, 'r', encoding='utf-8') as f:
            ibm_data = json.load(f)
            data['ibm_deps'] = ibm_data.get(service_name, {})
            
            # Collect all fields from ibm_deps
            # IBM has both 'independent' and 'dependent' arrays, plus 'operations' array
            for op_list_name in ['independent', 'dependent', 'operations']:
                operations = data['ibm_deps'].get(op_list_name, [])
                for op in operations:
                    ...
                    if isinstance(item_fields, dict):
                        for field_name in item_fields.keys():
                            ...
                            if normalized not in data['field_mapping']:
                                data['field_mapping'][normalized] = []
                            data['field_mapping'][normalized].append(('ibm_deps', field_name))
...
def get_possible_values(field_name: str, service_data: Dict) -> tuple:
    ...
    # Check ibm_deps
    for source, orig_name in original_names:
        if source == 'ibm_deps' and service_data['ibm_deps']:
            for op_list_name in ['independent', 'dependent', 'operations']:
                operations = service_data['ibm_deps'].get(op_list_name, [])
                for op in operations:
                    item_fields = op.get('item_fields', {})
                    if isinstance(item_fields, dict) and orig_name in item_fields:
                        ...
                        if field_data.get('enum') and 'possible_values' in field_data:
                            return (field_data['possible_values'], 'ibm_deps')
...
def generate_all_services(base_dir: Path):
    ...
    print("="*80)
    print("GENERATING FIELD-OPERATOR-VALUE TABLES FOR ALL IBM SERVICES")
    print("="*80)
