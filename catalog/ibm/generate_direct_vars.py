#!/usr/bin/env python3
"""
Generate direct_vars.json for IBM Cloud services from operation_registry.json.
...
def generate_discovery_id(service_name: str, operation_id: str) -> str:
    ...
    # Operation ID is like "get_account_settings" - already snake_case for IBM
    op_snake = operation_id if '_' in operation_id else to_snake_case(operation_id)
    return f"ibm.{service_name}.{op_snake}"

def extract_field_name_from_entity(entity: str, service_name: str) -> str:
    """Extract field name from entity path"""
    # IBM Entity format: "ibm.service.entity_name" (e.g., "ibm.iam.account_settings_assignment.account_settings_assignment_id")
    # Extract field name (entity_name part)
    parts = entity.split('.')
    
    if len(parts) >= 3 and parts[0] == "ibm" and parts[1] == service_name:
        # Remove "ibm.service" prefix, keep the rest as field name
        field_name = '.'.join(parts[2:])
        return field_name
    elif len(parts) >= 2 and parts[1] == service_name:
        # Format: "ibm.service.field" - take last part
        return parts[-1]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate direct_vars.json for IBM Cloud services")
    ...
    print("="*80)
    print("GENERATING DIRECT_VARS.JSON FOR IBM CLOUD SERVICES")
    print("="*80)
    print()
