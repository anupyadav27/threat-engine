#!/usr/bin/env python3
"""
Generate field_operator_value_table.csv for Azure services.
Adapted from AWS version for Azure structure.
"""
...
def load_service_data(service_name: str, base_dir: Path) -> Dict[str, Any]:
    ...
    # Load azure_dependencies_with_python_names_fully_enriched.json
    azure_deps_path = service_dir / 'azure_dependencies_with_python_names_fully_enriched.json'
    if azure_deps_path.exists():
        with open(azure_deps_path, 'r', encoding='utf-8') as f:
            azure_data = json.load(f)
            data['azure_deps'] = azure_data.get(service_name, {})
            
            # Collect all fields from azure_deps
            ops_by_cat = data['azure_deps'].get('operations_by_category', {})
            for category, category_data in ops_by_cat.items():
                for op_type in ['independent', 'dependent']:
                    ops = category_data.get(op_type, [])
                    for op in ops:
...
def get_possible_values(field_name: str, service_data: Dict) -> tuple:
    ...
    # Check azure_deps
    if service_data['azure_deps']:
        ops_by_cat = service_data['azure_deps'].get('operations_by_category', {})
        for category, category_data in ops_by_cat.items():
            for op_type in ['independent', 'dependent']:
                ops = category_data.get(op_type, [])
                for op in ops:
                    ...
                    if isinstance(item_fields, dict) and field_name in item_fields:
                        ...
                        if field_data.get('enum') and 'possible_values' in field_data:
                            return (field_data['possible_values'], 'azure_deps')
        ...
        for category, category_data in ops_by_cat.items():
            for op_type in ['independent', 'dependent']:
                ...
                for op in ops:
                    item_fields = op.get('item_fields', {})
                    if isinstance(item_fields, dict) and field_name in item_fields:
                        ops_list = item_fields[field_name].get('operators', [])
                        if ops_list:
...
def is_enum_field(field_name: str, service_data: Dict) -> bool:
    ...
    # Check azure_deps for enum flag
    if service_data['azure_deps']:
        ops_by_cat = service_data['azure_deps'].get('operations_by_category', {})
        for category, category_data in ops_by_cat.items():
            for op_type in ['independent', 'dependent']:
...
def generate_all_services(base_dir: Path):
    ...
    print("="*80)
    print("GENERATING FIELD-OPERATOR-VALUE TABLES FOR ALL AZURE SERVICES")
    print("="*80)
