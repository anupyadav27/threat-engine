#!/usr/bin/env python3
"""
Generate field_operator_value_table.csv for Kubernetes (k8s) services.
Adapted from OCI version for k8s structure.
"""
...
def load_service_data(service_name: str, base_dir: Path) -> Dict[str, Any]:
    ...
    # Load k8s_dependencies_with_python_names_fully_enriched.json
    k8s_deps_path = service_dir / 'k8s_dependencies_with_python_names_fully_enriched.json'
    if k8s_deps_path.exists():
        with open(k8s_deps_path, 'r', encoding='utf-8') as f:
            k8s_data = json.load(f)
            data['k8s_deps'] = k8s_data.get(service_name, {})
            
            # Collect all fields from k8s_deps
            # k8s has 'independent' array (and potentially 'dependent')
            for op_list_name in ['independent', 'dependent']:
                operations = data['k8s_deps'].get(op_list_name, [])
                for op in operations:
                    ...
                    if isinstance(item_fields, dict):
                        for field_name in item_fields.keys():
                            ...
                            if normalized not in data['field_mapping']:
                                data['field_mapping'][normalized] = []
                            data['field_mapping'][normalized].append(('k8s_deps', field_name))
...
def get_possible_values(field_name: str, service_data: Dict) -> tuple:
    ...
    # Check k8s_deps
    for source, orig_name in original_names:
        if source == 'k8s_deps' and service_data['k8s_deps']:
            for op_list_name in ['independent', 'dependent']:
                operations = service_data['k8s_deps'].get(op_list_name, [])
                for op in operations:
                    item_fields = op.get('item_fields', {})
                    if isinstance(item_fields, dict) and orig_name in item_fields:
                        ...
                        if field_data.get('enum') and 'possible_values' in field_data:
                            return (field_data['possible_values'], 'k8s_deps')
...
def generate_all_services(base_dir: Path):
    ...
    print("="*80)
    print("GENERATING FIELD-OPERATOR-VALUE TABLES FOR ALL K8S SERVICES")
    print("="*80)
