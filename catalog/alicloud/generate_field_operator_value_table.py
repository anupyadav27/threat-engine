#!/usr/bin/env python3
"""
Generate field_operator_value_table.csv for Alibaba Cloud (alicloud) services.
Adapted from Azure version for alicloud structure.
"""
...
def load_service_data(service_name: str, base_dir: Path) -> Dict[str, Any]:
    ...
    # Load alicloud_dependencies_with_python_names_fully_enriched.json
    alicloud_deps_path = service_dir / 'alicloud_dependencies_with_python_names_fully_enriched.json'
    if alicloud_deps_path.exists():
        with open(alicloud_deps_path, 'r', encoding='utf-8') as f:
            alicloud_data = json.load(f)
            data['alicloud_deps'] = alicloud_data.get(service_name, {})
            
            # Collect all fields from alicloud_deps (PascalCase)
            operations = data['alicloud_deps'].get('operations', [])
            for op in operations:
                ...
                if isinstance(item_fields, dict):
                    for field_name in item_fields.keys():
                        ...
                        if normalized not in data['field_mapping']:
                            data['field_mapping'][normalized] = []
                        data['field_mapping'][normalized].append(('alicloud_deps', field_name))
...
def get_possible_values(field_name: str, service_data: Dict) -> tuple:
    ...
    # Check alicloud_deps
    for source, orig_name in original_names:
        if source == 'alicloud_deps' and service_data['alicloud_deps']:
            operations = service_data['alicloud_deps'].get('operations', [])
            for op in operations:
                item_fields = op.get('item_fields', {})
                if isinstance(item_fields, dict) and orig_name in item_fields:
                    ...
                    if field_data.get('enum') and 'possible_values' in field_data:
                        return (field_data['possible_values'], 'alicloud_deps')
...
def is_enum_field(field_name: str, service_data: Dict) -> bool:
    ...
    # Check alicloud_deps for enum flag
    field_mapping = service_data.get('field_mapping', {})
    original_names = field_mapping.get(field_name, [])
...
def generate_all_services(base_dir: Path):
    ...
    print("="*80)
    print("GENERATING FIELD-OPERATOR-VALUE TABLES FOR ALL ALICLOUD SERVICES")
    print("="*80)
