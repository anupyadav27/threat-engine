    ...
    def __init__(
        ...
        if connection_string:
            ...
            self.connection_string = connection_string or os.getenv(
                "DATABASE_URL",
                f"postgresql://{db_user or os.getenv('DB_USER', 'postgres')}:"
                f"{db_password or os.getenv('DB_PASSWORD', '')}@"
                f"{db_host or os.getenv('DB_HOST', 'localhost')}:"
                f"{db_port or os.getenv('DB_PORT', '5432')}/"
                f"{db_name or os.getenv('DB_NAME', 'compliance')}"
            )
