...
def _update_orchestration(scan_run_id: str, **kwargs) -> None:
    ...
    values = list(kwargs.values()) + [scan_run_id]
    try:
        conn = _get_orch_conn()
        ...
        logger.warning(f"Failed to update orchestration {scan_run_id}: {e}")
...
def _get_orchestration(scan_run_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(f"Failed to read orchestration {scan_run_id}: {e}")
        return None


def _add_engine_completed(scan_run_id: str, engine: str) -> None:
...
async def run_pipeline(scan_run_id: str) -> None:
    ...
    """
    _update_orchestration(scan_run_id, overall_status="running")

    # ── Stage 0: Discovery ────────────────────────────────────────────
    logger.info("[%s] Stage 0: discovery", scan_run_id[:8])
    try:
        resp = await handlers.trigger_discovery(scan_run_id)
        logger.info("[%s] discovery complete", scan_run_id[:8])
        _add_engine_completed(scan_run_id, "discovery")
    except Exception as exc:
        logger.error("[%s] discovery FAILED: %s", scan_run_id[:8], exc)
    ...
    logger.info("[%s] Stage 1: check + inventory", scan_run_id[:8])
    stage1_results = await asyncio.gather(
        handlers.trigger_check(scan_run_id),
        handlers.trigger_inventory(scan_run_id),
        return_exceptions=True,
    )

    check_result = stage1_results[0]
    if isinstance(check_result, Exception):
        logger.error("[%s] check FAILED: %s", scan_run_id[:8], check_result)
    else:
        logger.info("[%s] check complete", scan_run_id[:8])
        _add_engine_completed(scan_run_id, "check")

    inv_result = stage1_results[1]
    if isinstance(inv_result, Exception):
        logger.error("[%s] inventory FAILED: %s", scan_run_id[:8], inv_result)
    else:
        logger.info("[%s] inventory complete", scan_run_id[:8])
        _add_engine_completed(scan_run_id, "inventory")
    ...
    # ── Stage 2: Threat ───────────────────────────────────────────────
    logger.info("[%s] Stage 2: threat", scan_run_id[:8])
    try:
        resp = await handlers.trigger_threat(scan_run_id)
        logger.info("[%s] threat complete", scan_run_id[:8])
        _add_engine_completed(scan_run_id, "threat")
    except Exception as exc:
        logger.error("[%s] threat FAILED: %s", scan_run_id[:8], exc)
        # Continue to compliance anyway — it uses scan_run_id

    # ── Stage 3: Compliance + IAM + DataSec (parallel) ────────────────
    logger.info("[%s] Stage 3: compliance + iam + datasec", scan_run_id[:8])
    ...
    stage3_results = await asyncio.gather(
        handlers.trigger_compliance(scan_run_id),
        handlers.trigger_iam(scan_run_id, csp=provider),
        handlers.trigger_datasec(scan_run_id, csp=provider),
        return_exceptions=True,
    )
    ...
    for name, result in zip(stage3_names, stage3_results):
        if isinstance(result, Exception):
            logger.error("[%s] %s FAILED: %s", scan_run_id[:8], name, result)
        else:
            logger.info("[%s] %s complete", scan_run_id[:8], name)
            _add_engine_completed(scan_run_id, name)
    ...
    logger.info("[%s] pipeline COMPLETE", scan_run_id[:8])
...
async def trigger_pipeline(request: PipelineRequest):
    ...
    if orch.get("overall_status") == "running":
        raise HTTPException(
            status_code=409,
            detail=f"Scan {request.scan_run_id} is already running",
        )

    asyncio.create_task(run_pipeline(request.scan_run_id))
...
async def get_pipeline_status(scan_run_id: str):
    ...
    return {
        "scan_run_id": str(orch["scan_run_id"]),
