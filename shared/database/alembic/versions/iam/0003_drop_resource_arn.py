...
2. Drops the idx_iam_findings_resource index (on resource_arn)
3. Drops the resource_arn column from iam_findings
"""
...
def upgrade():
    ...
    conn.execute(text("DROP INDEX IF EXISTS idx_iam_findings_resource"))

    # ── Step 3: Drop resource_arn column ───────────────────────────────────
    conn.execute(text(
        "ALTER TABLE iam_findings DROP COLUMN IF EXISTS resource_arn"
    ))
...
def downgrade():
    conn = op.get_bind()
    conn.execute(text(
        "ALTER TABLE iam_findings ADD COLUMN IF NOT EXISTS resource_arn TEXT"
    ))
    conn.execute(text(
        "CREATE INDEX IF NOT EXISTS idx_iam_findings_resource "
        "ON iam_findings(resource_arn)"
    ))
