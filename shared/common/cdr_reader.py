"""
Shared CDR Reader — reads log_events + cdr_findings for any engine.
...
Usage:
    from engine_common.cdr_reader import CDRReader

    reader = CDRReader(tenant_id, account_id)
...
class CDRReader:
    """Shared reader for log_events + cdr_findings data."""
    ...
    def _resolve_latest_cdr_scan_run_id(self) -> Optional[str]:
        """Return the scan_run_id of the latest completed CDR scan for this tenant.
        ...
        try:
            ...
            logger.warning(f"CDRReader._resolve_latest_cdr_scan_run_id failed: {exc}")
            return None
        finally:
            conn.close()
    ...
    def get_identity_usage(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            logger.warning(f"CDRReader.get_identity_usage failed: {exc}")
        finally:
            conn.close()
        return usage
    ...
    def get_cross_account_access(self) -> List[Dict]:
        ...
        try:
            ...
            logger.warning(f"CDRReader.get_cross_account_access failed: {exc}")
        finally:
            conn.close()
        return results
    ...
    def get_threat_events(self, min_severity: str = "low") -> List[Dict]:
        ...
        try:
            ...
            logger.warning(f"CDRReader.get_threat_events failed: {exc}")
        finally:
            conn.close()
        return results
    ...
    def _get_applicable_rule_ids(self, scope_col: str) -> Optional[List[str]]:
        ...
        try:
            ...
            logger.warning(f"CDRReader._get_applicable_rule_ids({scope_col}) failed: {exc}")
            return None

    def get_cdr_findings(self, engine_filter: str = "", enrich: bool = True) -> List[Dict]:
        """Get CDR findings, optionally filtered by engine scope.

        Args:
            engine_filter: Engine name (e.g. 'datasec', 'container', 'dbsec',
                'ai_security', 'network', 'iam'). Resolved to a rule_metadata
                JSONB scope column — only CDR rule_ids that are applicable to
        ...
        if engine_filter:
            ...
            if scope_col:
                ...
                if applicable_rule_ids is not None and len(applicable_rule_ids) == 0:
                    logger.info(f"No CDR-applicable rules for engine '{engine_filter}' — returning empty")
                    return []

        latest_scan_run_id = self._resolve_latest_cdr_scan_run_id()
        ...
        try:
            ...
            logger.warning(f"CDRReader.get_cdr_findings failed: {exc}")
        finally:
            conn.close()
    ...
    def _enrich_with_rule_metadata(self, findings: List[Dict]) -> List[Dict]:
        """Enrich CDR findings with rule_metadata from Check DB."""
        ...
        try:
            ...
            logger.warning(f"CDRReader._enrich_with_rule_metadata failed: {exc}")
        finally:
            if conn:
                conn.close()
        ...
        logger.info(f"Enriched {len(rule_cache)} / {len(rule_ids)} CDR rule_ids with rule_metadata")
        return findings
    ...
    def get_data_access_patterns(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            logger.warning(f"CDRReader.get_data_access_patterns failed: {exc}")
        finally:
            conn.close()
        return patterns
    ...
    def get_audit_completeness(self) -> Dict[str, Dict]:
        ...
        try:
            ...
            logger.warning(f"CDRReader.get_audit_completeness failed: {exc}")
        finally:
            conn.close()
        return completeness
    ...
    def get_network_events(self) -> List[Dict]:
        ...
        try:
            ...
            logger.warning(f"CDRReader.get_network_events failed: {exc}")
        finally:
            conn.close()
        return results
