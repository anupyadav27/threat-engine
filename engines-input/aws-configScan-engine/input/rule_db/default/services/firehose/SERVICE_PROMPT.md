# FIREHOSE Service Processing Prompt

## Task
Process FIREHOSE service metadata mapping and YAML generation following this sequence:

1. **Review all metadata from FIREHOSE**
   - Review all metadata files in `/Users/apple/Desktop/threat-engine/aws_compliance_python_engine/services/firehose/metadata/`
   - Understand security requirements and intent

2. **Create metadata_mapping.json**
   - Use boto3 database files from `/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/firehose/`
   - Reference:
...
3. **Review as AWS Python developer and security expert**
   - Verify methods exist in boto3 FIREHOSE client
...
4. **Generate YAML from metadata_mapping.json**
   - Run: `cd /Users/apple/Desktop/threat-engine/aws_compliance_python_engine && python3 generate_yaml_from_metadata_mapping.py firehose`
   - Verify YAML file is created in `services/firehose/rules/firehose.yaml`
...
5. **MANDATORY: Test with actual AWS resources**
   ...
   **Option B: Create test resources (if none exist)**
   - **REQUIRED**: Create a test script: `test_firehose_resources.py`
   ...
   - The test script MUST:
     ...
     2. **Run compliance scan** automatically:
        ...
        subprocess.run([
            ...
            '--service', 'firehose', '--region', 'us-east-1'
        ])
        ```
   ...
   - Run the test script:
     ```bash
     cd /Users/apple/Desktop/threat-engine/aws_compliance_python_engine
     python3 test_firehose_resources.py
     ```
   ...
   **Step 5c: Run compliance scan**
   ...
   python3 -m aws_compliance_python_engine.engine.main_scanner --service firehose --region us-east-1
   ```
   ...
   **Step 5e: Analyze scan results**
   ...
   find $LATEST_SCAN -name "*.json" -path "*/firehose/*" | head -5 | xargs cat
   ```
...
6. **Detailed output analysis (MANDATORY)**
   ...
   **6c: Verify field paths in check evidence**
   ...
   find $LATEST_SCAN -name "*.json" -path "*/firehose/*" | head -3 | while read file; do
       echo "Checking: $file"
...
7. **Fix issues and retest (iterative process)**
   ...
   **7a: Fix identified issues**
   ...
   - If discovery errors: Fix `rules/firehose.yaml` directly or fix `metadata_mapping.json`
   ...
   **7b: Regenerate YAML (if metadata_mapping.json changed)**
   ```bash
   cd /Users/apple/Desktop/threat-engine/aws_compliance_python_engine
   python3 generate_yaml_from_metadata_mapping.py firehose
   ```
