#!/usr/bin/env python3
"""
Validate CLOUDFORMATION YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "Stacks[].Capabilities" with items_for -> "item.Capabilities"
    - field_path: "Capabilities" with items_for -> "item.Capabilities"
    - field_path: "StackEvents[].ResourceStatus" with items_for -> "item.ResourceStatus"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('Stacks.'):
            clean_path = clean_path.replace('Stacks.', '')
        elif clean_path.startswith('StackEvents.'):
            clean_path = clean_path.replace('StackEvents.', '')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('cloudformation_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'cloudformation.yaml'
