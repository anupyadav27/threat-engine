...
def parse_compliance_frameworks(compliance_list: List[str]) -> Dict[str, bool]:
    """Parse compliance strings to identify which frameworks are present."""
    frameworks_found = {
        "hipaa": False,
        "pci": False,
        "gdpr": False
    ...
    for compliance_str in compliance_list:
        compliance_lower = compliance_str.lower()
        
        # Identify HIPAA
        if "hipaa" in compliance_lower:
            frameworks_found["hipaa"] = True
        
        # Identify PCI (look for pci, pci_dss, payment_card)
        if any(keyword in compliance_lower for keyword in ["pci", "pci_dss", "payment_card", "cardholder"]):
            frameworks_found["pci"] = True
        
        # Identify GDPR (look for gdpr, eu_gdpr, data_protection, eu_data_protection)
        if any(keyword in compliance_lower for keyword in ["gdpr", "eu_gdpr", "data_protection", "eu_data_protection"]):
            frameworks_found["gdpr"] = True
    
    return frameworks_found


def get_compliance_impact(rule_id: str, modules: List[str], metadata: Dict) -> Dict[str, str]:
    """Get compliance impact from existing compliance section in metadata."""
    impact = {}
    
    # Extract compliance frameworks from metadata
    compliance_list = metadata.get("compliance", [])
    frameworks_found = parse_compliance_frameworks(compliance_list)
    
    # If no specific frameworks found, fall back to generic mappings
    if not any(frameworks_found.values()):
    ...
    # Generate impact based on actual frameworks found and modules
    if frameworks_found["hipaa"]:
        if "data_protection_encryption" in modules:
            impact["hipaa"] = "§164.312(a)(2)(iv) - Encryption of ePHI at rest"
        elif "data_access_governance" in modules:
            impact["hipaa"] = "§164.312(a)(1) - Access control"
        elif "data_activity_monitoring" in modules:
            impact["hipaa"] = "§164.312(b) - Audit controls"
    
    if frameworks_found["pci"]:
        if "data_protection_encryption" in modules:
            impact["pci"] = "Requirement 3.4 - Render PAN unreadable via encryption"
        elif "data_access_governance" in modules:
            impact["pci"] = "Requirement 7 - Restrict access to cardholder data"
        elif "data_activity_monitoring" in modules:
            impact["pci"] = "Requirement 10 - Track and monitor all access"
    
    if frameworks_found["gdpr"]:
        if "data_protection_encryption" in modules:
            impact["gdpr"] = "Article 32 - Encryption requirement for personal data"
        elif "data_access_governance" in modules:
            impact["gdpr"] = "Article 25 - Data protection by design and by default"
        elif "data_activity_monitoring" in modules:
            impact["gdpr"] = "Article 30 - Records of processing activities"
...
def generate_data_security_section(rule_id: str, metadata: Dict, modules: List[str]) -> Dict:
    ...
    # Get compliance impact (using existing compliance section from metadata)
    impact = get_compliance_impact(rule_id, modules, metadata)
...
def find_metadata_files(rule_db_path: Path, service: Optional[str] = None) -> List[Path]:
    ...
    # Try two path structures:
    # 1. {rule_db_path}/default/services/{service}/metadata/
    # 2. {rule_db_path}/{service}/metadata/ (direct services structure)
    
    if service:
        # Try structure 1 first
        service_dir = rule_db_path / "default" / "services" / service
        if not service_dir.exists():
            # Try structure 2
            service_dir = rule_db_path / service
        ...
        if not services_dir.exists():
            # Try structure 2 (direct services directory)
            services_dir = rule_db_path
...
def main():
    ...
    parser.add_argument(
        "--rule-db-path",
        type=str,
        default="/Users/apple/Desktop/threat-engine/engine_check/engine_check_aws/services",
        help="Path to services directory (or rule_db path with default/services structure)"
    )
    parser.add_argument(
        "--service",
        type=str,
