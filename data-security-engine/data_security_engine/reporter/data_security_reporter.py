...
from ..input.configscan_reader import ConfigScanReader
...
class DataSecurityReporter:
    """Generates comprehensive data security reports."""
    
    def __init__(self, rule_db_path: Optional[str] = None, engines_output_path: Optional[str] = None):
        """
        ...
        """
        self.configscan_reader = ConfigScanReader(engines_output_path)
    ...
    def generate_report(
        ...
        # Load configScan findings - filtered by rule_id for efficiency
        logger.info(f"Loading configScan findings for scan: {scan_id} (filtered by {len(data_security_rule_ids)} data security rule IDs)")
        data_findings = self.configscan_reader.filter_data_related_findings(
            csp, scan_id, 
            data_security_rule_ids=data_security_rule_ids,  # Filter by rule_id FIRST
            max_findings=max_findings
        )
        
        # Load data stores from inventory
        logger.info("Loading data stores from inventory")
        data_stores = self.configscan_reader.filter_data_stores(csp, scan_id)
...
def generate_data_security_report(
    ...
    engines_output_path: Optional[str] = None
) -> Dict[str, Any]:
    """Generate a data security report."""
    reporter = DataSecurityReporter(rule_db_path, engines_output_path)
    return reporter.generate_report(csp, scan_id)
