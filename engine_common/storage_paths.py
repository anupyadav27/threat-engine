...
Uses relative paths from project root; override via WORKSPACE_ROOT.
"""
import os
...
def get_project_root() -> Path:
    """Project root (repo root). Prefer WORKSPACE_ROOT env if set."""
    root = os.getenv("WORKSPACE_ROOT")
    if root:
        return Path(root)
    # engine_common/storage_paths.py -> repo root
...
class StoragePathResolver:
    ...
    def __init__(
        ...
        Args:
            ...
            local_base_path: Local base path (default: project root / engine_output)
        """
        ...
        if local_base_path:
            self.local_base_path = str(local_base_path) if isinstance(local_base_path, Path) else local_base_path
        else:
            root = get_project_root()
            self.local_base_path = str(root / "engine_output")
    ...
    def get_scan_results_path(
        ...
        """
        # Standardized path format: engine_configscan_{csp}/output/{scan_run_id}/{filename}
        folder = f"engine_configscan_{csp}"
        relative_path = f"{folder}/output/{scan_run_id}/{filename}"
    ...
    def get_scan_directory(
        ...
        """
        folder = f"engine_configscan_{csp}"
        relative_path = f"{folder}/output/{scan_run_id}"
