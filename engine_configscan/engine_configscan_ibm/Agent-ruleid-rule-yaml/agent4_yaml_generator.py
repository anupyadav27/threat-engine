...
def generate_discovery_section(service: str, resource: str, operation: str, validated_op: Dict) -> Dict:
    """Generate discovery section for IBM"""
    discovery_id = f"ibm.{service}.{operation}"
    ...
    discovery = {
        ...
        'emit': {
            'items_for': f'{{{{ {operation}_response.items }}}}',  # IBM typically uses 'items'
            'as': 'item',
            'item': {}
        }
    }
...
def generate_check_section(requirement: Dict) -> Dict:
    ...
    for field_req in ai_reqs.get('fields', []):
        ibm_field = field_req.get('ibm_api_field', '')
        operator = field_req.get('operator', 'equals')
        expected_value = field_req.get('ibm_api_field_expected_values')
        
        # Validate field exists
        field_validation = requirement.get('field_validation', {}).get(ibm_field, {})
        if not field_validation.get('exists'):
            continue
        
        condition = {
            'var': f'item.{ibm_field}',
            'op': operator
        }
...
def main():
    logger.info("Agent 4: IBM YAML Generator starting")
    print("=" * 80)
    print("AGENT 4: IBM YAML Generator")
    print("=" * 80)
    print()
