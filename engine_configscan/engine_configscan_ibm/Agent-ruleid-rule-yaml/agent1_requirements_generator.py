"""
Agent 1: IBM Cloud Requirements Generator (AI-Powered)
"""
...
SERVICES_TO_PROCESS = ['vpc', 'iam', 'object_storage', 'key_protect', 'containers']
...
def get_metadata_files(service: str):
    ...
    for file in os.listdir(metadata_dir):
        if file.endswith('.yaml') and file.startswith('ibm.'):
            files.append(os.path.join(metadata_dir, file))
    
    return files


def generate_requirements_with_ai(rule_id: str, requirement: str, description: str, client, service: str, ibm_data: dict):
    """Use OpenAI GPT-4o to generate intelligent requirements"""
    
    service_data = ibm_data.get(service, {})
    ...
    prompt = f"""You are an IBM Cloud compliance expert analyzing a security rule.

Rule ID: {rule_id}
Requirement: {requirement}
Description: {description}

AVAILABLE IBM CLOUD API FIELDS FOR {service.upper()}:
{available_fields_summary}

TASK: Determine the EXACT field(s) to check from the IBM API list above.
...
{{
  "fields": [{{
    ...
    "ibm_api_field": "actual_field_name",
    "operator": "equals",
    "ibm_api_field_expected_values": value
  }}],
...
"""
    ...
    try:
        response = client.chat.completions.create(
            ...
            temperature=0.1,
            messages=[
                {"role": "system", "content": "You are an IBM Cloud compliance expert. Respond ONLY with valid JSON."},
        ...
        return {
            "fields": [{"conceptual_name": "basic_check", "ibm_api_field": "id", "operator": "exists", "ibm_api_field_expected_values": None}],
            "condition_logic": "single"
        }
...
def main():
    ...
    print("Loading IBM Cloud SDK catalog...")
    with open('ibm_sdk_catalog_enhanced.json') as f:
        ibm_data = json.load(f)
    print("✅ Loaded")
    
    print("=" * 80)
    print("AGENT 1: IBM Cloud Requirements Generator")
    print("=" * 80)
    ...
    for service in SERVICES_TO_PROCESS:
        ...
        for idx, metadata_file in enumerate(metadata_files, 1):
            ...
            ai_reqs = generate_requirements_with_ai(rule_id, requirement, description, client, service, ibm_data)
