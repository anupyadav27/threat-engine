"""
FastAPI server for GCP Compliance Engine - Enhanced
"""
import sys
import os
...
logger = setup_logger(__name__, engine_name="configscan-gcp")

app = FastAPI(
    title="GCP Compliance Engine API",
    description="API for running GCP compliance scans",
    version="1.0.0"
)

# Add logging middleware
app.add_middleware(CorrelationIDMiddleware)
app.add_middleware(RequestLoggingMiddleware, engine_name="configscan-gcp")
    ...
    with LogContext(tenant_id=request.tenant_id, scan_run_id=scan_id):
        try:
            ...
            if request.credentials:
                ...
                if cred_type == 'gcp_service_account':
                    logger.info("Using GCP Service Account credentials")
                    service_account_json = request.credentials.get('service_account_json')
                    if isinstance(service_account_json, str):
                        service_account_info = json.loads(service_account_json)
                    else:
                        service_account_info = service_account_json
                    
                    credential_override = service_account.Credentials.from_service_account_info(
...
async def list_scans(
    ...
    for scan_id, scan_data in scans.items():
        if status and scan_data.get("status") != status:
            continue
        if project and scan_data.get("project") != project:
            continue
