#!/usr/bin/env python3
"""
Validate LAMBDA YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "Functions[].FunctionName" with items_for -> "item.FunctionName"
    - field_path: "VpcConfig.VpcId" -> "item.VpcConfig.VpcId"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('Functions.'):
            clean_path = clean_path.replace('Functions.', '')
        elif clean_path.startswith('Layers.'):
            clean_path = clean_path.replace('Layers.', '')
        elif clean_path.startswith('LayerVersions.'):
            clean_path = clean_path.replace('LayerVersions.', '')
        elif clean_path.startswith('CodeSigningConfigs.'):
            clean_path = clean_path.replace('CodeSigningConfigs.', '')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('lambda_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'lambda.yaml'
