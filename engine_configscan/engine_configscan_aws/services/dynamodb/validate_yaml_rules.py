#!/usr/bin/env python3
"""
Validate DYNAMODB YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "SSEDescription" with item emit -> "item.SSEDescription"
    - field_path: "ContinuousBackupsDescription.ContinuousBackupsStatus" with item emit -> "item.ContinuousBackupsStatus"
    - field_path: "Table[].SSEDescription" with items_for -> "item.SSEDescription"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('Table.'):
            clean_path = clean_path.replace('Table.', '')
        elif clean_path.startswith('ContinuousBackupsDescription.'):
            clean_path = clean_path.replace('ContinuousBackupsDescription.', '')
        elif clean_path.startswith('GlobalTableDescription.'):
            clean_path = clean_path.replace('GlobalTableDescription.', '')
        elif clean_path.startswith('TableAutoScalingDescription.'):
            clean_path = clean_path.replace('TableAutoScalingDescription.', '')
        elif clean_path.startswith('BackupSummaries[].'):
            clean_path = clean_path.replace('BackupSummaries[].', '')
        elif clean_path.startswith('GlobalTables[].'):
            clean_path = clean_path.replace('GlobalTables[].', '')
        return f"item.{clean_path}" if clean_path else "item"
    else:
        # If not items_for, response is 'item', so use as-is
        if clean_path.startswith('Table.'):
            clean_path = clean_path.replace('Table.', '')
        elif clean_path.startswith('ContinuousBackupsDescription.'):
            clean_path = clean_path.replace('ContinuousBackupsDescription.', '')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('dynamodb_metadata_mapping', []):
    ...
    if expected_op == 'all':
        ...
        # Single condition, should not have all/any wrapper
        if 'all' in conditions or 'any' in conditions:
            issues.append('Expected single condition but found all/any wrapper')
    
    # Get conditions list
    if 'all' in conditions:
        condition_list = conditions['all']
    elif 'any' in conditions:
        condition_list = conditions['any']
    else:
    ...
    for i, expected_field in enumerate(expected_fields):
        ...
        # Check value (handle type conversions)
        ...
        if expected_value is not None:
            ...
            if isinstance(expected_value, bool) and isinstance(actual_value, str):
                ...
                # For 'in' operator, check if expected_value list matches
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'dynamodb.yaml'
