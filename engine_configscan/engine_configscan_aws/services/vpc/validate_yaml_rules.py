#!/usr/bin/env python3
"""
Validate VPC YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "Subnets[].name" with items_for -> "item.name"
    - field_path: "FlowLogs.FlowLogStatus" -> "item.FlowLogStatus"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('Subnets.'):
            clean_path = clean_path.replace('Subnets.', '')
        elif clean_path.startswith('CustomerGateways.'):
            clean_path = clean_path.replace('CustomerGateways.', '')
        elif clean_path.startswith('Addresses.'):
            clean_path = clean_path.replace('Addresses.', '')
        elif clean_path.startswith('Vpcs.'):
            clean_path = clean_path.replace('Vpcs.', '')
        elif clean_path.startswith('AccountAttributes.'):
            clean_path = clean_path.replace('AccountAttributes.', '')
        return f"item.{clean_path}" if clean_path else "item"
    else:
        # If not items_for, check if emit extracts from a specific path
        # Remove common prefixes
        prefixes = ['Subnets.', 'CustomerGateways.', 'Addresses.', 'Vpcs.', 'AccountAttributes.',
                   'EgressOnlyInternetGateways.', 'ServiceConfigurations.', 'FlowLogs.',
                   'SecurityGroupRules.', 'IpamResourceDiscoveries.', 'NatGateways.',
                   'ClientVpnEndpoints.', 'CarrierGateways.', 'ConnectionNotificationSet.',
                   'RouteTables.', 'NetworkAcls.', 'VpcEndpoints.', 'CapacityBlockExtensions.']
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('vpc_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'vpc.yaml'
