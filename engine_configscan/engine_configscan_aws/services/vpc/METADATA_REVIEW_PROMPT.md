# vpc Metadata Review and Consolidation Prompt

You are an expert AWS compliance engineer reviewing metadata mappings and metadata files for the **vpc** service. Your task is to analyze and provide actionable suggestions for consolidations and cross-service placements.

## Service Context

**Service Name**: `vpc`  
**Service Path**: `/Users/apple/Desktop/threat-engine/aws_compliance_python_engine/services/vpc/`
...
**Path**: `/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/vpc/boto3_dependencies_with_python_names_fully_enriched.json`

Contains actual AWS API method specifications to validate method ownership.

## Your Task

Analyze the vpc service and provide:
...
{{
  ...
  "consolidation_suggestions": {{
    "duplicates": [
      {{
        "keep": {{
          "rule_id": "aws.vpc.resource.check_complete",
          "metadata_file": "aws.vpc.resource.check_complete.yaml",
        ...
        "remove": [
          {{
            "rule_id": "aws.vpc.resource.check_basic",
            "metadata_file": "aws.vpc.resource.check_basic.yaml",
            "replaced_by": "aws.vpc.resource.check_complete",
  ...
  "cross_service_suggestions": [
    {{
      "rule_id": "aws.vpc.resource.check_name",
      "metadata_file": "aws.vpc.resource.check_name.yaml",
      "current_service": "vpc",
...
2. **Service Prefix Matching**
   - Extract service from rule_id: `aws.vpc.{{resource}}.{{check}}`
...
1. **Load all files** for vpc service
...
**Begin Review**: Analyze the vpc service files and generate comprehensive review JSON.
