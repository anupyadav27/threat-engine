#!/usr/bin/env python3
"""
Validate XRAY YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "SamplingRuleRecords[].SamplingRule.RuleName" with items_for -> "item.RuleName"
    - field_path: "EncryptionConfig.Type" -> "item.Type"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('SamplingRuleRecords.'):
            clean_path = clean_path.replace('SamplingRuleRecords.', '')
        # Remove nested SamplingRule prefix if present (since emit extracts directly)
        if clean_path.startswith('SamplingRule.'):
            clean_path = clean_path.replace('SamplingRule.', '')
        return f"item.{clean_path}" if clean_path else "item"
    else:
        # If not items_for, check if emit extracts from a specific path
        if clean_path.startswith('EncryptionConfig.'):
            clean_path = clean_path.replace('EncryptionConfig.', '')
        return f"item.{clean_path}" if clean_path else "item"
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('xray_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'xray.yaml'
