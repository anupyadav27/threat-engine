#!/usr/bin/env python3
"""
Validate ELASTIC YAML rules against metadata_mapping.json
"""
import json
import yaml
import os
...
def normalize_field_path(field_path, emit_structure):
    ...
    Examples:
    - field_path: "DomainStatusList[].SnapshotOptions" with items_for -> "item.SnapshotOptions"
    """
    ...
    if uses_items_for:
        ...
        if clean_path.startswith('DomainStatusList.'):
            clean_path = clean_path.replace('DomainStatusList.', '')
...
def validate_rule(rule, metadata_mapping, yaml_data):
    ...
    mapping = None
    for m in metadata_mapping.get('elastic_metadata_mapping', []):
...
def main():
    ...
    yaml_file = base_dir / 'rules' / 'elastic.yaml'
