#!/usr/bin/env python3
"""
SQS Metadata Review Analysis Script
Analyzes sqs service metadata for consolidation and cross-service placement opportunities.
"""
...
BOTO3_DEPS_FILE = Path("/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/sqs/boto3_dependencies_with_python_names_fully_enriched.json")
...
def check_method_ownership(method: str) -> Tuple[Optional[str], int]:
    ...
    try:
        ...
        # Check if method exists in sqs service
        sqs_data = boto3_data.get("sqs", {})
        sqs_ops = sqs_data.get("independent", []) + sqs_data.get("dependent", [])
        sqs_has_method = any(
        ...
        for service_dir in boto3_base.iterdir():
            if not service_dir.is_dir() or service_dir.name == "sqs":
                continue
        ...
        if sqs_has_method:
            if service_count > 0:
                return "sqs", service_count + 1  # Ambiguous
            return "sqs", 1  # Only in sqs
        elif service_count > 0:
            return first_other_service, service_count  # Not in sqs, in other service(s)
        else:
...
def extract_service_from_rule_id(rule_id: str) -> str:
    """Extract service name from rule_id (e.g., aws.sqs.queue.x -> sqs)"""
    parts = rule_id.split(".")
...
def main():
    ...
    rules = mapping_data.get("sqs_metadata_mapping", [])
    if not rules:
