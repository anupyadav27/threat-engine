#!/usr/bin/env python3
"""
WAFv2 Metadata Review Analysis Script
Analyzes wafv2 service metadata for consolidation and cross-service placement opportunities.
"""
...
BOTO3_DEPS_FILE = Path("/Users/apple/Desktop/threat-engine/pythonsdk-database/aws/wafv2/boto3_dependencies_with_python_names_fully_enriched.json")
...
def check_method_ownership(method: str) -> Tuple[Optional[str], int]:
    ...
    try:
        ...
        # Check if method exists in wafv2 service
        wafv2_ops = boto3_data.get("wafv2", {}).get("operations", [])
        wafv2_has_method = any(
        ...
        if wafv2_has_method:
            if service_count > 0:
                return "wafv2", service_count + 1  # Ambiguous
            return "wafv2", 1  # Only in wafv2
        elif service_count > 0:
            return first_other_service, service_count  # Not in wafv2, in other service(s)
        else:
...
def extract_service_from_rule_id(rule_id: str) -> str:
    """Extract service name from rule_id (e.g., aws.wafv2.webacl.x -> wafv2)"""
    parts = rule_id.split(".")
...
def analyze_cross_service(mapping_data: List[Dict], metadata: Dict[str, Dict]) -> List[Dict]:
    ...
    for rule in mapping_data:
        ...
        if method_service and method_service != "wafv2":
        ...
        elif method_service == "wafv2" and service_count > 1:
            # Method exists in wafv2 but also in other services (ambiguous)
            suggestions.append({
                "rule_id": rule_id,
                "metadata_file": f"{rule_id}.yaml",
                "current_service": "wafv2",
                "suggested_service": "wafv2",
                "reason": f"Method {method} exists in wafv2 but also in {service_count-1} other service(s) - verify correct usage",
                "python_method": method,
...
def main():
    ...
    with open(METADATA_MAPPING_FILE, 'r') as f:
        mapping_data = json.load(f)["wafv2_metadata_mapping"]
    ...
    cross_service_count = len([s for s in cross_service if s.get("suggested_service") != "wafv2"])
    ...
    if cross_service_count > 0:
        cross_service_confidence = sum(
            ...
            if s.get("suggested_service") != "wafv2"
        ) / cross_service_count
    else:
