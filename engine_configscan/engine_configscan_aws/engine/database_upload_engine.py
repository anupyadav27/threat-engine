...
class DatabaseUploadEngine:
    ...
    def upload_scan_to_database(self, scan_id: str, output_dir: Path,
        ...
        if not discovery_dir.exists():
            alt = output_dir / "discoveries" / scan_id / "discovery"
            discovery_dir = alt if alt.exists() else None

        if discovery_dir is None:
            logger.warning(
                f"No discovery directory for scan {scan_id} "
                f"(tried discovery/ and discoveries/{scan_id}/discovery/)"
            )
            return {
                "total_files": 0,
                "total_records": 0,
        ...
        # Find all NDJSON files (*_discoveries.ndjson or *.ndjson)
        ...
        # Update scan status (consolidated DB: use 'completed')
