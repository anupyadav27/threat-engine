"""
PostgreSQL Database Manager for Multi-Tenant CSMP Platform
Enhanced with centralized database management support
Supports both legacy psycopg2 and new centralized database connections
"""
import os
import json
import hashlib
...
# Note: Import path relative to engine_configscan_aws module root
import sys
import os
...
class DatabaseManager:
    ...
    def __init__(self, db_config: Optional[Dict] = None):
        """
        Initialize database connection using ONLY consolidated database system.
        ...
        try:
            db_config_obj = get_database_config("configscan")
            ...
            logger.info(f"Using consolidated configscan database: {db_config_obj.database} on {db_config_obj.host}")
        ...
        # Set schema search_path for engine_configscan and engine_shared
        self.search_path = os.getenv("DB_SCHEMA", "engine_configscan,engine_shared")
        self.use_consolidated_db = True  # Always using consolidated DB now
        
        self.connection_pool = None
        self._init_pool()
        # Schema is created via init-databases.sql, no need to ensure here

    def _load_db_config(self) -> Dict:
        """
        DEPRECATED: Load database config from local sources.
        This method is kept for backward compatibility but should not be used.
        Use consolidated database system instead.
        """
        logger.warning("_load_db_config() is deprecated. Use consolidated database system.")
        # Still provide fallback for emergency cases, but log warning
        try:
            db_config_obj = get_database_config("configscan")
            return {
                "host": db_config_obj.host,
            ...
            logger.error("Failed to load database config from consolidated system")
            raise
    ...
    def _init_pool(self):
        ...
        try:
            ...
            logger.info(
                ...
                + (" (consolidated DB)" if self.use_consolidated_db else "")
            )
        except Exception as e:
    ...
    def _get_connection(self):
        """Get connection from pool; set search_path for consolidated DB schemas."""
        if self.connection_pool is None:
            self._init_pool()
        conn = self.connection_pool.getconn()
        # Always set search_path when using consolidated DB
        if self.search_path:
            try:
                parts = [p.strip() for p in self.search_path.split(",") if p.strip()]
                if parts:
                    placeholders = ", ".join("%s" for _ in parts)
                    with conn.cursor() as cur:
                        cur.execute(
                            f"SET search_path TO {placeholders}",
                            tuple(parts),
                        )
            except Exception as e:
                self.connection_pool.putconn(conn)
                raise RuntimeError(f"Failed to set search_path: {e}") from e
        return conn
    ...
    def get_consolidated_connection(self):
        """Get connection using consolidated database configuration"""
        db_config_obj = get_database_config("configscan")
    ...
    def execute_with_best_config(self, query: str, params: Optional[tuple] = None):
        """Execute query using consolidated database config"""
        conn = None
        try:
            conn = self.get_consolidated_connection()
            ...
            if conn:
                conn.rollback()
            logger.error(f"Centralized config query failed: {e}")
            raise
        finally:
            if conn:
                conn.close()
    
    def fetch_one_with_best_config(self, query: str, params: Optional[tuple] = None):
        """Fetch one row using centralized config if available, otherwise legacy"""
        if self.use_centralized and CENTRALIZED_DB_AVAILABLE:
            conn = None
            try:
                conn = self.get_centralized_connection()
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    if params:
                        cur.execute(query, params)
                    else:
                ...
                logger.error(f"Centralized config fetch_one failed: {e}")
                raise
            finally:
                if conn:
                    conn.close()
    ...
    def get_database_info(self) -> Dict[str, Any]:
        ...
        return {
            "use_centralized": self.use_centralized,
            "centralized_available": CENTRALIZED_DB_AVAILABLE,
            "database": self.db_config.get("database"),
            "host": self.db_config.get("host"),
            "port": self.db_config.get("port"),
            "user": self.db_config.get("user"),
            "connection_type": "centralized" if self.use_centralized else "legacy"
        }

    def _ensure_schema(self):
        """Ensure database schema exists (legacy; skipped when using consolidated DB)."""
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                for item in items:
                    ...
                    history_batch.append((
                        ...
                        discovery_id, history_resource_arn, resource_uid, scan_id, config_hash,
