"""
FastAPI server for Azure Compliance Engine
"""
import sys
import os
...
logger = setup_logger(__name__, engine_name="configscan-azure")

app = FastAPI(
    title="Azure Compliance Engine API",
    description="API for running Azure compliance scans",
    version="1.0.0"
)

# Add logging middleware
app.add_middleware(CorrelationIDMiddleware)
app.add_middleware(RequestLoggingMiddleware, engine_name="configscan-azure")
    ...
    with LogContext(tenant_id=request.tenant_id, scan_run_id=scan_id):
        logger.info("Received scan request", extra={
            "extra_fields": {
                ...
                "locations": request.include_locations,
        ...
        scans[scan_id] = {
            ...
            "locations": request.include_locations or [],
    ...
    with LogContext(tenant_id=request.tenant_id, scan_run_id=scan_id):
        try:
            ...
            if request.credentials:
                ...
                if cred_type == 'azure_service_principal':
                    logger.info("Using Azure Service Principal credentials")
                    credential_override = ClientSecretCredential(
        ...
        results = scan(
            ...
            exclude_subscriptions=request.exclude_subscriptions,
...
async def list_scans(
    ...
    for scan_id, scan_data in scans.items():
        if status and scan_data.get("status") != status:
            continue
        if subscription and scan_data.get("subscription") != subscription:
            continue
        filtered_scans.append({
            "scan_id": scan_id,
            "status": scan_data.get("status"),
            "subscription": scan_data.get("subscription"),
