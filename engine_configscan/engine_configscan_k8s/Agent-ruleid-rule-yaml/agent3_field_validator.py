"""
Agent 3: K8s Field Validator

Validates that fields exist in K8s API responses and corrects field names.
...
def check_field_in_nested(field_path: str, available_fields: Dict) -> Dict:
    ...
    Args:
        ...
        available_fields: Available fields dict
    ...
    for i, part in enumerate(parts):
        if part in current:
            if i == len(parts) - 1:
                ...
                # Go deeper
                nested_fields = current[part].get('nested_fields', {})
                if nested_fields:
                    current = nested_fields
                else:
                    return {'exists': False, 'reason': f'No nested_fields in {part}'}
        else:
            return {'exists': False, 'reason': f'Field {part} not found'}
...
def validate_fields(requirement: Dict, catalog: Dict) -> Dict:
    ...
    resource_data = resource_data.get('resources', {}).get(resource_name, {})
    ...
    for field_req in requirement.get('ai_generated_requirements', {}).get('fields', []):
        field_name = field_req.get('k8s_api_field', '')
        
        validation_result = check_field_in_nested(field_name, available_fields)
...
def main():
    logger.info("Agent 3: K8s Field Validator starting")
    print("=" * 80)
    print("AGENT 3: K8s Field Validator")
    print("=" * 80)
    print()
    ...
    for resource_name, requirements in all_requirements.items():
        print(f"\n📦 {resource_name}")
        logger.info(f"Validating fields for resource: {resource_name}")
